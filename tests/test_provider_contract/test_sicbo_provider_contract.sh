#!/bin/bash
# test_sicbo_provider_contract.sh — SUN-1339 C1
#
# Provider-contract e2e tests for Sicbo.
# Mirrors TaiXiu structure exactly.
...
#   After a Sicbo round is settled, add:
#     - POST /api/v2/admin/sicbo/unsettle { roundId: <settled_roundId> }
...
if [[ -z "$PLAYER_TOKEN" ]]; then
    echo -e "${YELLOW}WARN: Player login (c=3) unavailable on staging — all Sicbo tests will be SKIPPED${RESET}"
fi

# ══════════════════════════════════════════════════════
section "SICBO — state endpoint"
# ══════════════════════════════════════════════════════

test_name "GET /api/v2/sicbo/state → log safeBetExpiresAt + roundId; skip if engine idle"
if [[ -z "$PLAYER_TOKEN" ]]; then
    _skip "No player token — c=3 login unavailable on staging"
else
    STATE_RESP=$(curl -sf --max-time 15 \
        -H "Authorization: Bearer ${PLAYER_TOKEN}" \
        "${BASE}/api/v2/sicbo/state" 2>&1 \
...
" 2>/dev/null)
    ...
    if [[ "$STATE_HTTP_STATUS" == "403" || "$STATE_HTTP_STATUS" == "404" ]]; then
        ...
        _skip "engine idle — safeBetExpiresAt=0 and roundId empty; Sicbo may not be running"
        ROUND_ID=""
...
# ══════════════════════════════════════════════════════
section "SICBO — bet endpoint (side=Tai, amount=1000)"
# ══════════════════════════════════════════════════════

BET_EC=""

test_name "POST /api/v2/sicbo/bet (side=Tai, amount=1000) → success"
if [[ -z "$PLAYER_TOKEN" ]]; then
    _skip "No player token — c=3 login unavailable on staging"
elif [[ -z "${ROUND_ID:-}" ]]; then
    _skip "engine idle or state unavailable — skipping bet test"
else
    NONCE="test-sicbo-$(date +%s)-$$"
...
" 2>/dev/null)
    ...
    if [[ "$BET_EC" == "BET_WINDOW_CLOSED" ]]; then
        _skip "BET_WINDOW_CLOSED — Sicbo bet window not open for round=${ROUND_ID}"
...
# ══════════════════════════════════════════════════════
section "SICBO — admin/unsettle on PENDING (live-round) bet → 400 NOT_SETTLED"
# ══════════════════════════════════════════════════════

test_name "POST /api/v2/admin/sicbo/unsettle (pending bet) → 400 NOT_SETTLED"
if [[ -z "$PLAYER_TOKEN" ]]; then
    _skip "No player token — c=3 login unavailable on staging"
elif [[ -z "${ROUND_ID:-}" ]]; then
    ...
    UNSETTLE_RESP=$(curl -s --max-time 15 \
        ...
        -d "$UNSETTLE_PAYLOAD" \
        "${BASE}/api/v2/admin/sicbo/unsettle" 2>&1 \
        || echo '{"__httpcode":0}')
...
# ══════════════════════════════════════════════════════
# TODO(SUN-1339 C2): post-settle path
#   Requires a Sicbo round with settle_status == SETTLED.
#   Steps to add:
#     1. Confirm round settled: GET state → roundStatus == SETTLED (or DB check)
#     2. POST /api/v2/admin/sicbo/unsettle { roundId: <settled_roundId> }
