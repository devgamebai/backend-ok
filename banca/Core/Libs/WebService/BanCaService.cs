        ...
        public BanCaService()
        {
            ...
            Get("/bancaapi/addCash/{secret}/{userid}/{changeCash}/{reasonText}", async parameters =>
            {
                ...
                if (secret == _secret && (allowIps.Count == 0 || (allowIps.Count > 0 && allowIps.Contains(this.Request.UserHostAddress))))
                {
                    try
                    {
                        // Phase 5c — CMS addCash admin via unified wallet.
                        if (MoneyGatewayClient.IsEnabled())
                        {
                            var sid = "bc-cms-addcash-" + userid + "-" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                            var tx = changeCash >= 0 ? "WAGER_CREDIT_BANCA" : "WAGER_DEBIT_BANCA";
                            bool ok = false;
                            try { ok = await MoneyGatewayClient.SettleAsync(userid, changeCash, sid, "EMERGENCY_BANCA"); }
                            catch (Exception ex) { Logger.Error("Phase5c CMS addCash settle err uid=" + userid + " " + ex.Message); }
                            newCash = ok ? changeCash : -1;
                        }
                        else
                        {
                        ...
                        MySqlUser.SaveCashToDb(userid, newCash); // no-op under Phase 5c
            ...
            Post("/bancaapi/updateCash/{secret}/{userid}/{changeCash}/{reasonText}", async parameters =>
            {
                ...
                if (secret == _secret && (allowIps.Count == 0 || (allowIps.Count > 0 && allowIps.Contains(this.Request.UserHostAddress))))
                {
                    try
                    {
                        // Phase 5c — CMS updateCash admin via unified wallet.
                        if (MoneyGatewayClient.IsEnabled())
                        {
                            var sid = "bc-cms-updcash-" + userid + "-" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                            bool ok = false;
                            try { ok = await MoneyGatewayClient.SettleAsync(userid, changeCash, sid, "EMERGENCY_BANCA"); }
                            catch (Exception ex) { Logger.Error("Phase5c CMS updateCash settle err uid=" + userid + " " + ex.Message); }
                            newCash = ok ? changeCash : -1;
                        }
                        else
                        {
                    ...
                    try
                    {
                        ...
                        if ("real" == mode)
                        {
                            ...
                            if (smsItem != null)
                            {
                                ...
                                if (MoneyGatewayClient.IsEnabled())
                                {
                                    var sid = "bc-pay-sms-" + userId + "-" + totalAmount + "-" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                                    bool ok = false;
                                    try { ok = await MoneyGatewayClient.SettleAsync(userId, smsItem.Cash, sid, "EMERGENCY_BANCA"); }
                                    catch (Exception ex) { Logger.Error("Phase5c SMS_IN settle err uid=" + userId + " " + ex.Message); }
                                    newCash = ok ? smsItem.Cash : -1;
                                }
                                else
                                {
                                    newCash = await RedisManager.IncEpicCash(userId, smsItem.Cash, "system",
                ...
                try
                {
                    ...
                    if (authKey != null && md5.Equals(authKey) && message != null && message.StartsWith("scv", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        try
                        {
                            ...
                            if (MoneyGatewayClient.IsEnabled())
                            {
                                var sid = "bc-pay-momo-" + momo_transId + "-" + userId;
                                bool ok = false;
                                try { ok = await MoneyGatewayClient.SettleAsync(userId, add, sid, "EMERGENCY_BANCA"); }
                                catch (Exception ex) { Logger.Error("Phase5c MOMO_IN settle err uid=" + userId + " " + ex.Message); }
                                newCash = ok ? add : -1;
                            }
                            else
                            {
                                newCash = await RedisManager.IncEpicCash(userId, add, "momo " + momo_transId + " " + message + " " + phone, reasonText, TransType.MOMO_IN, realmoney: rm, rate: config.MomoCashInRate);
                            }
                            if (newCash < 0) // error
                            {
                    ...
                    if (EpicApi.Md5(apiKey + tranid).Equals(signature))
                    {
                        ...
                        try
                        {
                            ...
                            if (status == 3)
                            {
                                ...
                                if (MoneyGatewayClient.IsEnabled())
                                {
                                    var sid = "bc-pay-muacard-" + tranid + "-" + userId;
                                    bool ok = false;
                                    try { ok = await MoneyGatewayClient.SettleAsync(userId, add, sid, "EMERGENCY_BANCA"); }
                                    catch (Exception ex) { Logger.Error("Phase5c CARD_IN muacard settle err uid=" + userId + " " + ex.Message); }
                                    newCash = ok ? add : -1;
                                }
                                else
                                {
                                    newCash = await RedisManager.IncEpicCash(userId, add, "system",
                    ...
                    if (EpicApi.Md5(apiKey + tranid + cardSerial + cardType + menhGiaNhapVao + statuscode).Equals(sign))
                    {
                        ...
                        try
                        {
                            ...
                            if (statuscode == "00")
                            {
                                ...
                                if (MoneyGatewayClient.IsEnabled())
                                {
                                    var sid = "bc-pay-push247-" + tranid + "-" + userId;
                                    bool ok = false;
                                    try { ok = await MoneyGatewayClient.SettleAsync(userId, add, sid, "EMERGENCY_BANCA"); }
                                    catch (Exception ex) { Logger.Error("Phase5c CARD_IN push247 settle err uid=" + userId + " " + ex.Message); }
                                    newCash = ok ? add : -1;
                                }
                                else
                                {
                    ...
                    try
                    {
                        ...
                        if (json["status"].AsInt == 1)
                        {
                            try
                            {
                                ...
                                if (MoneyGatewayClient.IsEnabled())
                                {
                                    var sid = "bc-pay-cardcharge-" + userId + "-" + add + "-" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                                    bool ok = false;
                                    try { ok = await MoneyGatewayClient.SettleAsync(userId, add, sid, "EMERGENCY_BANCA"); }
                                    catch (Exception ex) { Logger.Error("Phase5c CARD_IN cardcharge settle err uid=" + userId + " " + ex.Message); }
                                    newCash = ok ? add : -1;
                                }
                                else
                                {
                                    newCash = await RedisManager.IncEpicCash(userId, add, "webapi", reasonText, TransType.CARD_IN);
                                }
                                if (newCash < 0) // error
                                {
                                    return "{\"status\":0, \"message\":\"fail, code " + newCash + "\"}";
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            if (statuscode == 0)
                            {
                                ...
                                if (MoneyGatewayClient.IsEnabled())
                                {
                                    var sid = "bc-pay-vinpay-" + tranid + "-" + userId;
                                    bool ok = false;
                                    try { ok = await MoneyGatewayClient.SettleAsync(userId, add, sid, "EMERGENCY_BANCA"); }
                                    catch (Exception ex) { Logger.Error("Phase5c CARD_IN vinpay settle err uid=" + userId + " " + ex.Message); }
                                    newCash = ok ? add : -1;
                                }
                                else
                                {
                ...
                if (data.HasKey("status"))
                {
                    ...
                    if (status == 2 || status >= 100) // ok
                    {
                        try
                        {
                            ...
                            if (MoneyGatewayClient.IsEnabled())
                            {
                                var sid = "bc-pay-coin-" + userid + "-" + data["txn_id"].Value;
                                bool ok = false;
                                try { ok = await MoneyGatewayClient.SettleAsync(userid, changeCash, sid, "EMERGENCY_BANCA"); }
                                catch (Exception ex) { Logger.Error("Phase5c COIN_PAYMENT_IPN settle err uid=" + userid + " " + ex.Message); }
                                newCash = ok ? changeCash : -1;
                            }
                            else
                            {
                                newCash = await RedisManager.IncEpicCash(userid, changeCash, "coin,add:" + data["address"].Value + ",amount:" + data["amount"].Value,
