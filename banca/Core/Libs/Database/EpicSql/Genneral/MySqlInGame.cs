    ...
    public class MySqlInGame
    {
        ...
        public static async Task<Slot5Histories> GetSlot5Histories(long user_id, string gamename)
        {
            ...
            try
            {
                ...
                {
                    using (MySqlCommand cmd = new MySqlCommand("SELECT trans_id,blind,win_lines,slot,total_bet,win_cash,create_time,request_lines FROM slot5_histories WHERE user_id = @userId AND gamename = @gamename order by trans_id desc LIMIT 50;", con))
                    {
        ...
        public static async Task<Slot3SubHistories> GetSlot3SubHistories(long user_id, string gamename)
        {
            ...
            try
            {
                using (MySqlConnection con = MySqlConnect.Connection())
                {
                    using (MySqlCommand cmd = new MySqlCommand("SELECT trans_id,blind,win_lines,slot,win_cash,create_time FROM slot3_histories WHERE user_id=@userId AND gamename=@gamename order by trans_id desc LIMIT 50;", con))
                    {
                        cmd.Parameters.AddWithValue("@userId", user_id);
                        cmd.Parameters.AddWithValue("@gamename", gamename);
                        cmd.CommandType = CommandType.Text;
        ...
        public static async Task<BigSmallHistories> GetBigSmallHistories(long userId, int limit = 50)
        {
            ...
            var sql = @"SELECT `big_small_transactions`.`game_session`,
...
FROM `big_small_transactions` INNER JOIN `big_small_histoies` ON `big_small_transactions`.`game_session`=`big_small_histoies`.`trans_id`AND `big_small_transactions`.`user_id`=@userId
ORDER BY `big_small_transactions`.`game_session` DESC LIMIT @limit";
            try
            {
                using (MySqlConnection con = MySqlConnect.Connection())
                {
        ...
        public static async Task<TopGames> GetTopGameSlot5(string gamename)
        {
            ...
            try
            {
                using (MySqlConnection con = MySqlConnect.Connection())
                {
                    using (MySqlCommand cmd = new MySqlCommand("SELECT trans_id, blind, nickname, win_cash, create_time, description FROM slot5_glory WHERE gamename=@gamename ORDER BY trans_id DESC LIMIT 50", con))
                    {
                        cmd.Parameters.AddWithValue("@gamename", gamename);
                        cmd.CommandType = CommandType.Text;
                        await con.OpenAsync();
        ...
        public static async Task<TopGames> GetTopGameSlot3Sub(string gamename)
        {
            ...
            try
            {
                using (MySqlConnection con = MySqlConnect.Connection())
                {
                    using (MySqlCommand cmd = new MySqlCommand("SELECT trans_id, blind, nickname, win_cash, create_time, description FROM slot3_glory WHERE gamename=@gamename ORDER BY trans_id DESC LIMIT 50", con))
                    {
                        cmd.Parameters.AddWithValue("@gamename", gamename);
                        cmd.CommandType = CommandType.Text;
                        await con.OpenAsync();
