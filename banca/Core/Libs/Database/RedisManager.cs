...
namespace BanCa.Redis
{
    ...
    public enum TransType : int
    {
        ...
        SLOT3_WIN = 46,
        SMS_IN = 47,
        ONE_TWO_THREE_PAY = 48,
        ONE_TWO_THREE_WIN = 49,
    ...
    public class RedisManager
    {
        ...
        public const string
            DEVICE_ID_USERID = "bc_deviceid_userid"; // hash map, hold unique <device id, first account userid>
        //public const string DEVICE_ID_USERID2 = "bc_deviceid2_userid"; // hash map, hold unique <device id, first account userid>

        private static ConcurrentDictionary<int, ConcurrentDictionary<int, int>> freeSlotCache =
            new ConcurrentDictionary<int, ConcurrentDictionary<int, int>>(); // <server, <blind index, free slot>>
        ...
        // Phase 5c follow-up — per-user expiry timestamp (Unix-ms) for the
        // unified-wallet HTTP read cache. 5s default keeps the HUD smooth
        // without flooding c=9997.
        private static ConcurrentDictionary<long, long> userCashCacheExpiry = new ConcurrentDictionary<long, long>();
        private const long UNIFIED_CASH_CACHE_TTL_MS = 5000L;

        public static async Task<long> GetUserCash(long user_id)
        {
            // Phase 5c follow-up — when the unified wallet is on, the canonical
            // balance lives in PLAYER_VIN, not in Redis User_Cash:{id}. Hit
            // c=9997 (with a 5s in-memory cache) instead of Redis so per-frame
            // HUD reads see fresh values immediately after a settle. On any
            // HTTP failure we fall through to the legacy Redis path so a
            // backend-api outage degrades to stale-mirror rather than failed
            // reads.
            if (BanCa.Libs.UnifiedWallet.MoneyGatewayClient.IsEnabled())
            {
                var nowMs = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                if (userCashCacheExpiry.TryGetValue(user_id, out var expiry)
                    && nowMs < expiry
                    && userCashCache.TryGetValue(user_id, out var cachedMilli))
                {
                    return cachedMilli;
                }
                try
                {
                    var milli = await BanCa.Libs.UnifiedWallet.MoneyGatewayClient.QueryBalanceMilliAsync(user_id);
                    if (milli >= 0)
                    {
                        userCashCache[user_id] = milli;
                        userCashCacheExpiry[user_id] = nowMs + UNIFIED_CASH_CACHE_TTL_MS;
                        return milli;
                    }
                }
                catch (Exception ex)
                {
                    Logger.Warning("GetUserCash unified path threw user_id=" + user_id + " err=" + ex.Message);
                }
                // fall through to legacy Redis on HTTP failure
            }
        ...
        public static void LogBlindsServer(int id, int blindIndex, int ccu, int freeSlot, long profit)
        {
            ...
            {
                var redis = GetRedis();
                redis.HashSet(KEY_LOG_BLIND_SERVER + id, blindIndex, data,
                    flags: CommandFlags.FireAndForget); // no interest in response
            }
            catch (Exception ex)
            {
                Logger.Error("Redis error: " + ex.ToString());
        ...
        public static int GetBestFitServer(int blindIndex, bool useWs = false, List<int> servers = null,
            List<int> wsServers = null)
        {
            if (servers == null)
            {
                servers = ServerList;
        ...
        public static void UpdateCrashCash(string playerId, long cash)
        {
            ...
            {
                var redis = GetRedis();
                redis.HashSet(CRASH_PLAYER_CASH_BC, playerId, cash,
                    flags: CommandFlags.FireAndForget); // no interest in response
            }
            catch (Exception ex)
            {
        ...
        public static void LogCrashCash(IBatch batch, string playerId, long cash)
        {
            ...
            try
            {
                batch.HashSetAsync(CRASH_PLAYER_CASH_BC, playerId, str,
                    flags: CommandFlags.FireAndForget); // save cash at crash time
                batch.HashDeleteAsync(PLAYER_ONLINE_BC, playerId,
                    flags: CommandFlags.FireAndForget); // no longer online
            }
            catch (Exception ex)
            {
                Logger.Error("Redis error: " + ex.ToString());
        ...
        public static void IncTop(string username, long addCash)
        {
            ...
            try
            {
                ...
                batch.SortedSetIncrementAsync(PLAYER_CASH_WEEK_BC, username, addCash,
                    flags: CommandFlags.FireAndForget); // leaderboard
                batch.SortedSetIncrementAsync(PLAYER_CASH_MONTH_BC, username, addCash,
        ...
        public static async Task<JSONArray> GetTopPrize(bool isWeek)
        {
            var redis = GetRedis();
            var _prizes =
                await redis.StringGetAsync(isWeek
                    ? CURRENT_LEADERBOARD_WEEK_PRIZE_BC
                    : CURRENT_LEADERBOARD_MONTH_PRIZE_BC);
        ...
        public static async void SetTopPrize(bool isWeek, JSONArray prizes)
        {
            ...
            {
                var redis = GetRedis();
                await redis.StringGetSetAsync(
                    isWeek ? CURRENT_LEADERBOARD_WEEK_PRIZE_BC : CURRENT_LEADERBOARD_MONTH_PRIZE_BC, prizes.ToString());
            }
            catch (Exception ex)
            {
                Logger.Error("Redis error: " + ex.ToString());
        ...
        {
            var redis = GetRedis();
            var leaderboard = await redis.SortedSetRangeByRankWithScoresAsync(PLAYER_CASH_WEEK_BC, start: 0, stop: 50,
        ...
        {
            var redis = GetRedis();
            var leaderboard = await redis.SortedSetRangeByRankWithScoresAsync(PLAYER_CASH_MONTH_BC, start: 0, stop: 50,
                order: Order.Descending);
            var arr = new JSONArray();
        ...
        public static void LogHomeMessages(long userId, string nickname, long cashGain, FishType fishType,
            long tableIndex, BulletType bType, string gameId = "banca")
        {
            try
            {
                var redis = GetRedis();
        ...
        public static async Task<long> IncEpicCash(long user_id, long cash, string platform, string source,
            TransType reason, bool useCache = true, bool allowNegative = false, long realmoney = 0, float rate = 0)
        {
            // Phase 5c — wallet unification. When BANCA_USE_UNIFIED_WALLET=1
            // the Java MoneyGateway PLAYER_VIN ledger is the source of
            // truth. Every direct legacy caller of IncEpicCash has been
            // migrated to MoneyGatewayClient.SettleAsync (see Phase 5c
            // doc). If we still get here it is because:
            //   (a) a caller is on the legacy/flag-off branch (e.g. some
            //       GameBanCa solo-mode fallback) — historical behaviour
            //       is preserved.
            //   (b) a code path we missed during migration — log + treat
            //       as a Redis cache-only update so MySQL is never
            //       touched (the cgame.users.cash column was dropped in
            //       staging Phase 5e).
            // Either way we never call SaveCashToDb here because the
            // MySQL column is gone. Redis User_Cash:{id} remains as a
            // cache-only mirror that may drift during 5d shadow mode.
            try
            {
                var redis = GetRedis();
                string key = "User_Cash:" + user_id;
...

                {
                    ...
                    var config = LobbyConfig.GetDefaultConfig();
                    if (cash < 0 && CurrentCash < config.CashSaveMin && reason != TransType.CMS &&
                        reason != TransType.CASH_OUT &&
                        reason != TransType.TRANSFER_CASH && reason != TransType.MOMO_OUT &&
                        reason != TransType.BANK_OUT)
                    {
                        CheckCashSave(user_id, config.CashSaveMin, config.CashSaveAmount, platform,
                            reason == TransType.TAIXIU_BET ? 30000 : 10000);
                    }
        ...
        public static void SaveFund(Dictionary<int, double> profit, Dictionary<int, double> fund,
            Dictionary<int, double> bombFund, Dictionary<int, double> jackpot)
        {
            try
            {
                var redis = GetRedis();
        ...
        public static async void LoadFund(Dictionary<int, double> profit, Dictionary<int, double> fund,
            Dictionary<int, double> bombFund, Dictionary<int, double> jackpot)
        {
            var redis = GetRedis();
            try
            ...
            try
            {
                ...
                foreach (var item in jacks)
                {
                    ...
                    var index = key;
                    var min = (double) (Config.JackpotInitial *
                                        Config.GetBulletValue(index, (Config.BulletType) (key % 10)));
                ...
                {
                    jackpot[11] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(1, Config.BulletType.Bullet1));
                    jackpot[12] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(1, Config.BulletType.Bullet2));
                    jackpot[13] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(1, Config.BulletType.Bullet3));
                    jackpot[14] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(1, Config.BulletType.Bullet4));

                    jackpot[21] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(2, Config.BulletType.Bullet1));
                    jackpot[22] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(2, Config.BulletType.Bullet2));
                    jackpot[23] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(2, Config.BulletType.Bullet3));
                    jackpot[24] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(2, Config.BulletType.Bullet4));

                    jackpot[31] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(3, Config.BulletType.Bullet1));
                    jackpot[32] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(3, Config.BulletType.Bullet2));
                    jackpot[33] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(3, Config.BulletType.Bullet3));
                    jackpot[34] =
                        (double) (Config.JackpotInitial * Config.GetBulletValue(3, Config.BulletType.Bullet4));
                }
            }
            catch (Exception ex)
            {
        ...
        public static void SetItemTimestamp(string username, long snipeTimestamp, long rFireTimestamp)
        {
            try
            {
                var redis = GetRedis();
                username = username.Trim().ToLower();
                redis.StringSet(ITEM_SNIPE_BC + username, snipeTimestamp,
                    new TimeSpan(0, 0, (int) Config.SnipeCoolDownS), flags: CommandFlags.FireAndForget);
                redis.StringSet(ITEM_RAPID_FIRE_BC + username, rFireTimestamp,
                    new TimeSpan(0, 0, (int) Config.FastFireCoolDownS), flags: CommandFlags.FireAndForget);
            }
            catch (Exception ex)
            {
        ...
        public static async Task<int> SeeVideoAds(long userId, int type)
        {
            ...
            if (await redis.KeyExistsAsync(key)) // in day
            {
                ...
                {
                    if (type == 0) result = Config.VideoAdsRewardCount0 - 1;
                    else result = Config.VideoAdsRewardCount1 - 1;
            ...
            else // new day
            {
                ...
                var key0 = string.Format("{0}{1}_{2}", VIDEO_ADS_COUNT_BC, userId, 0);
                redis.StringSet(key0, type == 0 ? Config.VideoAdsRewardCount0 - 1 : Config.VideoAdsRewardCount0,
                    new TimeSpan(24, 0, 0), flags: CommandFlags.FireAndForget); // set

                var key1 = string.Format("{0}{1}_{2}", VIDEO_ADS_COUNT_BC, userId, 1);
                redis.StringSet(key1, type == 1 ? Config.VideoAdsRewardCount1 - 1 : Config.VideoAdsRewardCount1,
                    new TimeSpan(24, 0, 0), flags: CommandFlags.FireAndForget); // set
        ...
        public static void SaveBotConfig()
        {
            ...
            {
                var redis = GetRedis();
                redis.StringSet(BC_BOT_CONFIG, BanCa.Libs.Bots.BotConfig.ToJson().ToString(),
                    flags: CommandFlags.FireAndForget); // no interest in response
            }
            catch (Exception ex)
            {
                    ...
                    if (string.IsNullOrEmpty(time))
                    {
                        ...
                        if (userCash < min)
                        {
                            ...
                            if (amount > 0)
                            {
                                ...
                                long newCash;
                                // Phase 5c (SUN-1054) audit fix MAJOR 4 — gate the
                                // ADD_COIN_EVENT credit behind the unified wallet so
                                // the bonus actually shows up in the ledger.
                                if (MoneyGatewayClient.IsEnabled())
                                {
                                    var sid = "bc-add-coin-event-" + userId + "-" + TimeUtil.TimeStamp;
                                    try { await MoneyGatewayClient.SettleAsync(userId, amount, sid, "EMERGENCY_BANCA"); }
                                    catch (Exception ex) { Logger.Error("Phase5c ADD_COIN_EVENT settle err uid=" + userId + " " + ex.Message); }
                                    newCash = await GetUserCash(userId);
                                }
                                else
                                {
                                    newCash = await IncEpicCash(userId, amount, platform, "Add coin event",
                                        TransType.ADD_COIN_EVENT);
                                }
                                if (newCash > -1)
                                {
                                    user.Cash = newCash;
        ...
        public static void SetReloginEvent(int daysToGift, long giftCash, long minCashIn, string message)
        {
            try
            {
                ...
                batch.StringSetAsync(EventRelogMinCashInKey, minCashIn,
                    flags: CommandFlags.FireAndForget); // -1 = disable
        ...
        public static async Task<string> CheckReloginEvent(User user)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(days)) // has event
                {
                    ...
                    var lastLoginTime = string.IsNullOrEmpty(user.TimeLogin)
                        ? DateTime.Now
                        : DateTime.Parse(user.TimeLogin);
                    ...
                    if (daysL >= 0 && daysL <= totalTime)
                    {
                        ...
                        if (totalCashIn >= min)
                        {
                            ...
                            if (amount > 0)
                            {
                                ...
                                long newCash;
                                // Phase 5c (SUN-1054) audit fix MAJOR 4 — gate
                                // the RELOGIN_EVENT credit behind unified wallet.
                                if (MoneyGatewayClient.IsEnabled())
                                {
                                    var sid = "bc-relogin-event-" + userId + "-" + TimeUtil.TimeStamp;
                                    try { await MoneyGatewayClient.SettleAsync(userId, amount, sid, "EMERGENCY_BANCA"); }
                                    catch (Exception ex) { Logger.Error("Phase5c RELOGIN_EVENT settle err uid=" + userId + " " + ex.Message); }
                                    newCash = await GetUserCash(userId);
                                }
                                else
                                {
                                    newCash = await IncEpicCash(userId, amount, platform, "Add relogin event",
                                        TransType.RELOGIN_EVENT);
                                }
                                if (newCash > -1)
                                {
                                    user.Cash = newCash;
...
[3,3,10000]
        ...
        public static async Task<Tuple<long, int, long>> AcquireDailyFreeGoldItem(long userId)
        {
            ...
            if (gifts != null && gifts.Count > day)
            {
                ...
                if (todayGift.Count > 2)
                {
                    ...
                    long newCash;
                    // Phase 5c (SUN-1054) audit fix MAJOR 4 — gate the
                    // DAILY_GOLD daily-chest credit behind unified wallet.
                    if (MoneyGatewayClient.IsEnabled())
                    {
                        var sid = "bc-daily-gold-" + userId + "-" + TimeUtil.TimeStamp;
                        try { await MoneyGatewayClient.SettleAsync(userId, addCash, sid, "EMERGENCY_BANCA"); }
                        catch (Exception ex) { Logger.Error("Phase5c DAILY_GOLD settle err uid=" + userId + " " + ex.Message); }
                        newCash = await GetUserCash(userId);
                    }
                    else
                    {
                        newCash = await IncEpicCash(userId, addCash, "server", "daily chest", TransType.DAILY_GOLD);
                    }
        ...
        public static async Task<Tuple<int, int>> AcquireDailyFreeItem(long userId)
        {
            ...
            if (dayGoldArr.Count > day && dayGoldArr[day].AsInt == 0 && gifts != null && gifts.Count > day)
            {
                ...
                {
                    dayGoldArr[day] = 1;
                    redis.HashSet(ChestItemAccquireKey, userId, dayGoldArr.ToString(),
                        flags: CommandFlags.FireAndForget);
        ...
        public static async void CheckCashSave(long userId, long cashMin, long cashSave, string platform,
            int delayMs = 1000)
        {
            try
            {
                ...
                if (userCash > -1 && userCash < cashMin)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        if (!config.CheckVerifyPhone || (!string.IsNullOrEmpty(user.PhoneNumber) &&
                                                         !string.IsNullOrEmpty(user.DeviceId)))
                        {
                            var redis = GetRedis();
                            var key = config.CheckVerifyPhone
                                ? string.Format(CASH_SAVE_KEY, user.DeviceId, user.PhoneNumber)
                            ...
                            {
                                // ok
                                redis.StringSet(key, TimeUtil.TimeStamp, remaining, When.NotExists,
        ...
        public static async Task<JSONNode> GetMomoInfo()
        {
            try
            {
                ...
                if (json != null)
                    redis.StringSet(MoMoInfoKey, json.ToString(), TimeSpan.FromMinutes(5),
                        flags: CommandFlags.FireAndForget);
                return json;
            }
            catch (Exception ex)
