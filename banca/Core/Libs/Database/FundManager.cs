...
namespace BanCa.Libs
{
    public class FundManager
    {
        ...
        public static SimpleJSON.JSONNode GetJackpotJson()
        {
            ...
            lock (lockFund)
            {
                // SUN-536: Send real bombBank values to client instead of display jackpot
                foreach (var pair in bombBank)
                {
                    data[pair.Key.ToString()] = pair.Value;
                }
                // Include any jackpot keys not in bombBank (fallback)
                foreach (var pair in jackpot)
                {
                    if (!data.HasKey(pair.Key.ToString()))
                    {
                        data[pair.Key.ToString()] = pair.Value;
                    }
                }
        ...
        public static void IncJackpotFake()
        {
            lock (lockFund)
            {
                ...
                {
                    // Sync display jackpot with real accumulated bombBank value
                    if (bombBank.ContainsKey(key))
                    {
                        jackpot[key] = bombBank[key];
                    }
                }
            }
        }
        ...
        public static double GetJackpot(int blindIndex, Config.BulletType bulletType)
        {
            ...
            lock (lockFund)
            {
                // SUN-536: Return real bombBank value instead of display jackpot
                if (bombBank.ContainsKey(key))
                {
                    return bombBank[key];
                }
                // Fallback to display jackpot if bombBank not initialized yet
                if (jackpot.ContainsKey(key))
                {
                    return jackpot[key];
                }
