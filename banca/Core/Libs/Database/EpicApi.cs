...
namespace BanCa.Libs
{
    public class EpicApi
    {
        ...
        // SUN-1054: bridge to backend-api RealTimeCommission.
        //
        // Called from LobbyService.cs when the player withdraws from the
        // fish wallet back to the main xxeng wallet (bc => xxeng). Sends
        // one HTTP event per cashout. The backend-api processor at c=9854
        // records an audit row (banca_bet_commission_log) and — when
        // BANCA_COMMISSION_ENABLED=1 on the Java side — calls
        // RealTimeCommission.calculate exactly like Java games do, so the
        // commission cascade + rebate_logs writes happen through a single
        // implementation.
        //
        // Idempotency: sessionId is a BanCa-issued string unique per
        // cashout (we build it as "bc-{userId}-{unixMs}"). Safe under
        // network retry.
        //
        // Auth: X-Service-Token header, value from env BANCA_SERVICE_TOKEN.
        //
        // Fire-and-forget at the call site — a failure here must never
        // block the player's cashout. 5s timeout, exceptions swallowed.
        public static async Task<JSONNode> LogBetCommission(
            string nickname, string gameKey, long betAmount, string sessionId,
            long depositInVin, long withdrawOutVin, long netPlVin,
            long startedAtUnix, long endedAtUnix)
        {
            if (string.IsNullOrEmpty(nickname) || string.IsNullOrEmpty(sessionId) || betAmount < 0)
            {
                Logger.Warning("LogBetCommission: skipping — missing/invalid inputs nickname=" + nickname + " sessionId=" + sessionId + " betAmount=" + betAmount);
                return null;
            }
            var serviceToken = Environment.GetEnvironmentVariable("BANCA_SERVICE_TOKEN");
            if (string.IsNullOrEmpty(serviceToken))
            {
                Logger.Warning("LogBetCommission: BANCA_SERVICE_TOKEN not set, skipping");
                return null;
            }
            var backendUrl = ConfigJson.Config["xxeng-backend"].Value;
            if (string.IsNullOrEmpty(backendUrl))
            {
                Logger.Warning("LogBetCommission: xxeng-backend not configured, skipping");
                return null;
            }

            var payload = new JSONObject();
            payload["nickname"] = nickname;
            payload["game_key"] = string.IsNullOrEmpty(gameKey) ? "fish" : gameKey;
            payload["bet_amount"] = betAmount;
            payload["session_id"] = sessionId;
            payload["deposit_in_vin"] = depositInVin;
            payload["withdraw_out_vin"] = withdrawOutVin;
            payload["net_pl_vin"] = netPlVin;
            payload["started_at"] = startedAtUnix;
            payload["ended_at"] = endedAtUnix;
            ...
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var request = new HttpRequestMessage(HttpMethod.Post, backendUrl + "/api_backend?c=9854"))
                    {
                        request.Headers.TryAddWithoutValidation("X-Service-Token", serviceToken);
                        request.Content = new StringContent(payload.ToString(), Encoding.UTF8, "application/json");
                        var response = await httpClient.SendAsync(request);
                        var body = await response.Content.ReadAsStringAsync();
                        Logger.Info("LogBetCommission: session=" + sessionId + " status=" + (int)response.StatusCode + " body=" + body);
            ...
            {
                Logger.Error("LogBetCommission: HTTP call failed session=" + sessionId + " err=" + e.Message);
                return null;
            }
        }
        #endregion
