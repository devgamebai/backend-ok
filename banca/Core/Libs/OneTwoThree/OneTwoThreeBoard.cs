            ...
            runner.QueueAction(async () =>
            {
                try
                {
                    for (int i = 0, n = playRequests.Count; i < n; i++)
                    {
                        ...
                        {
                            // Phase 5c — OneTwoThree disconnect refund.
                            if (MoneyGatewayClient.IsEnabled())
                            {
                                var sid = "bc-123-refund-disc-" + pr.UserId + "-" + pr.Timestamp;
                                try { await MoneyGatewayClient.SettleAsync(pr.UserId, pr.Blind, sid, "WAGER_CREDIT_BANCA"); }
                                catch (Exception ex) { Logger.Error("Phase5c 123 refund-disc err uid=" + pr.UserId + " " + ex.Message); }
                            }
                            else
                            {
        ...
        public void PlayRequest(long userId, string nickname, string avatar, int blind, string clientId, Action<JSONNode> cb)
        {
            runner.QueueAction(async () =>
            {
                try
                {
                    ...
                    // Phase 5c — OneTwoThree bet debit via unified wallet.
                    long cash;
                    if (MoneyGatewayClient.IsEnabled())
                    {
                        var sid = "bc-123-bet-" + userId + "-" + TimeUtil.TimeStamp;
                        bool ok = false;
                        try { ok = await MoneyGatewayClient.SettleAsync(userId, -blind, sid, "WAGER_DEBIT_BANCA"); }
                        catch (Exception ex) { Logger.Error("Phase5c 123 bet settle err uid=" + userId + " " + ex.Message); }
                        cash = ok ? blind : -1;
                    }
                    else
                    {
                        cash = await RedisManager.IncEpicCash(userId, -blind, "server", "onetwothree", TransType.ONE_TWO_THREE_PAY);
                    }
                    if (cash >= 0)
                    {
            ...
            runner.QueueAction(async () =>
            {
                try
                {
                    ...
                    for (int i = 0, n = playRequests.Count; i < n; i++)
                    {
                        ...
                        {
                            // Phase 5c — OneTwoThree cancel refund.
                            if (MoneyGatewayClient.IsEnabled())
                            {
                                var sid = "bc-123-refund-cancel-" + userId + "-" + playRequests[i].Timestamp;
                                try { await MoneyGatewayClient.SettleAsync(userId, playRequests[i].Blind, sid, "WAGER_CREDIT_BANCA"); }
                                catch (Exception ex) { Logger.Error("Phase5c 123 refund-cancel err uid=" + userId + " " + ex.Message); }
                            }
                            else
                            {
