...
namespace Network
{
    ...
    public class WebsocketServer : NetworkServer
    {
        ...
        public int TimeOutMs = 60000; // SUN-855: was 5000ms — any peer idle >5s
                                      // got force-kicked even while actively playing
                                      // (boss fight pauses, reload animations, watching
                                      // a fish swim). Bumped to 60s (industry norm for
                                      // game WS idle kick). Real network drops still
                                      // caught instantly via TCP RST + OnPing/SendPong.
                                      // Proper long-term fix is FE-side 3s heartbeat
                                      // (tracked separately on SUN-855 comment).
