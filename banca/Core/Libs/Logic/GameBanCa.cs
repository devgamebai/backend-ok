    ...
    public class GameBanCa // = one game table
    {
        ...
        private void EndGame()
        {
            ...
            if (p1Cash == p2Cash && SoloPlayerId1.BombCount == SoloPlayerId2.BombCount) // draw
            {
                ...
                {
                    // Phase 5c — Solo end (draw refund). Two CREDIT calls.
                    if (MoneyGatewayClient.IsEnabled())
                    {
                        var sid1 = "bc-solo-end-draw-" + id1 + "-" + _id;
                        var sid2 = "bc-solo-end-draw-" + id2 + "-" + _id;
                        try { await MoneyGatewayClient.SettleAsync(id1, winCash, sid1, "WAGER_CREDIT_BANCA"); }
                        catch (Exception ex) { Logger.Error("Phase5c END_SOLO_BC_DRAW p1 settle err id=" + id1 + " " + ex.Message); }
                        try { await MoneyGatewayClient.SettleAsync(id2, winCash, sid2, "WAGER_CREDIT_BANCA"); }
                        catch (Exception ex) { Logger.Error("Phase5c END_SOLO_BC_DRAW p2 settle err id=" + id2 + " " + ex.Message); }
                    }
                    else
                    {
                        // refund (legacy)
            ...
            else
            {
                ...
                TaskRunner.RunOnPool(async () =>
                {
                    // Phase 5c — Solo end (winner takes pot).
                    if (MoneyGatewayClient.IsEnabled())
                    {
                        var sidW = "bc-solo-end-win-" + winId + "-" + _id;
                        try { await MoneyGatewayClient.SettleAsync(winId, winCash, sidW, "WAGER_CREDIT_BANCA"); }
                        catch (Exception ex) { Logger.Error("Phase5c END_SOLO_BC settle err winId=" + winId + " " + ex.Message); }
                    }
                    else
                    {
                                ...
                                if (o.Health <= 0)
                                {
...
#endif
                                    {
                                        if (p != null)
                                        {
                                            ...
                                            p.Exp += exp;
                                            p.ExpGain += exp;
                                            Logger.Info($"[trace] KILL player={p.Id} value={v} cash_after={p.Cash} profit_after={p.Profit} reason=kill-814");
                                        ...
                                        if (p != null)
                                        {
                                            ...
                                            if (index != -1 && index < mul.Count)
                                            {
...
#endif
                                                {
                                                    ...
                                                    p.Cash += v;
                                                    p.Profit += v;
                                                    Logger.Info($"[trace] KILL player={p.Id} value={v} cash_after={p.Cash} profit_after={p.Profit} reason=kill-942-goldenfrog");
                                                    p.PendingPushCash += v;
                                                    p.CashGain += v;
                                                    p.Exp += exp;
                                                    p.ExpGain += exp;
        ...
        public Player RegisterPlayer(string playerId, long epicId, long cash, bool isBot = false)
        {
            ...
            if (soloMode)
            {
                ...
                if (players[0] == null)
                {
                    ...
                    {
                        initFish();
                        // Phase 5c — Solo fee debit via unified wallet. Legacy
                        // path remains for BANCA_USE_UNIFIED_WALLET=0.
                        long soloRes;
                        if (MoneyGatewayClient.IsEnabled())
                        {
                            var sid = "bc-solo-fee-" + epicId + "-" + Id + "-p0";
                            bool ok = false;
                            try
                            {
                                ok = MoneyGatewayClient.SettleAsync(epicId, -cashToJoin, sid, "WAGER_DEBIT_BANCA").GetAwaiter().GetResult();
                            }
                            catch (Exception ex)
                            {
                                Logger.Error("Phase5c solo-fee p0 settle threw id=" + epicId + " err=" + ex.Message);
                            }
                            soloRes = ok ? 0 : -1;
                        }
                        else
                        {
                            soloRes = Redis.RedisManager.IncEpicCashCache(epicId, -cashToJoin, "server",
                ...
                else if (players[2] == null)
                {
                    ...
                    {
                        initFish();
                        // Phase 5c — Solo fee debit via unified wallet (player 2 slot).
                        long soloRes;
                        if (MoneyGatewayClient.IsEnabled())
                        {
                            var sid = "bc-solo-fee-" + epicId + "-" + Id + "-p2";
                            bool ok = false;
                            try
                            {
                                ok = MoneyGatewayClient.SettleAsync(epicId, -cashToJoin, sid, "WAGER_DEBIT_BANCA").GetAwaiter().GetResult();
                            ...
                            {
                                Logger.Error("Phase5c solo-fee p2 settle threw id=" + epicId + " err=" + ex.Message);
                            }
                            soloRes = ok ? 0 : -1;
                        }
                        else
            ...
            for (int i = 0, n = players.Length; i < n; i++)
            {
                if (players[i] != null && players[i].PlayerId == playerId)
                {
                    ...
                    if (IsServer && p != null && !p.IsBot)
                    {
                        ...
                        var profit = p.Profit;
                        long id = p.Id;
                        // Session-end marker for history bracket. Pair with the
                        // session-start trace emitted on the player's first shoot.
                        if (p.SessionStartedMs > 0)
                        {
                            var durMs = TimeUtil.TimeStamp - p.SessionStartedMs;
                            Logger.Info($"[trace] session-end player={id} world={Id} reason=leave durationMs={durMs} cash={cash} profit={profit}");
                            p.SessionStartedMs = 0;
                        }
                        // Phase 5b — wallet unification. When the kill switch
                        // BANCA_USE_UNIFIED_WALLET is on we settle the
                        // accumulated session profit to the Java MoneyGateway
                        // (PLAYER_VIN ledger) instead of Redis User_Cash. The
                        // legacy path below remains untouched for shadow mode
                        // (Phase 5d). Hot-path arithmetic (p.Cash / p.Profit)
                        // is NOT modified here — we only redirect the settle.
                        ...
                        TaskRunner.RunOnPool(async () =>
                        {
                            if (!soloMode)
                            {
                                if (useUnifiedWallet)
                                {
                                    var sessionId = "bc-" + id + "-" + Id;
                                    var txType = profit >= 0 ? "WAGER_CREDIT_BANCA" : "WAGER_DEBIT_BANCA";
                                    bool ok = false;
                                    Logger.Info($"[trace] RemovePlayer-settle player={id} profit={profit}");
                                    try
                                    {
                                        ok = await MoneyGatewayClient.SettleAsync(id, profit, sessionId, txType);
                                    }
                                    catch (Exception ex)
                                    {
                                        Logger.Error("Phase5b settle threw on QUIT_BC id=" + id + " err=" + ex.Message);
                                    }
                                    Logger.Info($"[trace] RemovePlayer-result player={id} ok={ok}");
                                    if (ok)
                                    {
                                        BanCa.Redis.RedisManager.IncTop(p.PlayerId, p.CashGain);
                                        BanCa.Redis.RedisManager.SavePlayer(p);
                                        // One bc_log row per play episode (close=leave).
                                        // Skip when profit==0: either the player never shot,
                                        // or the prior idle-tick already wrote the bracket
                                        // and RemovePlayer is just the empty followup.
                                        if (profit != 0)
                                        {
                                            try
                                            {
                                                BanCa.Sql.SqlLogger.LogSessionEnd(
                                                    Id, TableBlindIndex, id,
                                                    p.Cash, profit, ServerId,
                                                    sessionId, "leave");
                                            }
                                            catch (Exception logEx)
                                            {
                                                Logger.Warning("LogSessionEnd failed pid=" + id + " err=" + logEx.Message);
                                            }
                                        }
                                    }
                                    else
                                    {
                                        Logger.Warning("Phase5b QUIT_BC settle failed id=" + id + " profit=" + profit + " — dropped to banca:failed_settle");
                                    }
                                }
                                else
                                {
                        ...
                        if (soloMode) // save cash
                        {
                            if (!soloEnded)
                            {
                                ...
                                {
                                    var cashToJoin = Config.TableSoloCashIn[TableBlindIndex];
                                    // Phase 5c — Solo refund via unified wallet. Same
                                    // external_ref shape as the matching debit so the
                                    // server-side dedupe can correlate. CREDIT direction.
                                    if (MoneyGatewayClient.IsEnabled())
                                    {
                                        var sidR = "bc-solo-refund-" + p.Id + "-" + Id;
                                        var pid = p.Id;
                                        TaskRunner.RunOnPool(async () =>
                                        {
                                            try { await MoneyGatewayClient.SettleAsync(pid, cashToJoin, sidR, "WAGER_CREDIT_BANCA"); }
                                            catch (Exception ex) { Logger.Error("Phase5c solo-refund settle err id=" + pid + " " + ex.Message); }
                                        });
                                    }
                                    else
                                    {
            ...
            if (p != null && !Config.IsMaintain)
            {
                ...
                if (b != null)
                {
                    ...
                    if (p.Item != Config.PowerUp.FreeShot)
                    {
                        p.Profit -= _value;
                        p.Cash -= _value;
                        Logger.Info($"[trace] SHOOT player={p.Id} value={_value} cash_after={p.Cash} profit_after={p.Profit}");
                    }
                    ...
                    if (p.SessionStartedMs == 0)
                    {
                        p.SessionStartedMs = now;
                        Logger.Info($"[trace] session-start player={p.Id} world={this.Id} cash={p.Cash}");
                    }
                    if (OnShoot != null)
                    {
                        var msg = new JSONObject();
                        ...
                        if (o.Health <= 0)
                        {
                            var v = (long) (o.MaxHealth * _value / _power);
                            {
                                if (p != null)
                                {
                                    ...
                                    p.Exp += exp;
                                    p.ExpGain += exp;
                                    Logger.Info($"[trace] KILL player={p.Id} value={v} cash_after={p.Cash} profit_after={p.Profit} reason=kill-2478-bomb");
                                    if (!soloMode && p.DoLevelUpIfApplicable())
                                    {
                                        if (OnLevelUp != null)
                                        {
        ...
        public long KillAllFishInScreen(Player p, Config.BulletType type)
        {
            ...
            if (p != null)
            {
                ...
                p.Exp += exp;
                p.ExpGain += exp;
                Logger.Info($"[trace] KILL player={p.Id} value={killAllProfit} cash_after={p.Cash} profit_after={p.Profit} reason=kill-2817-killall");
                var mylv = p.Level;
                for (int i = 0, n = allObjects.Count; i < n; i++)
                {
                    var o = allObjects[i];
            ...
            if (IsServer && !soloMode)
            {
                ...
                if (hitTarget == FishType.Basic)
                {
                    ...
                    if (p != null)
                    {
                        p.Profit += b.Value;
                        p.Cash += b.Value;
                        Logger.Info($"[trace] KILL player={p.Id} value={b.Value} cash_after={p.Cash} profit_after={p.Profit} reason=kill-2978-refund");
