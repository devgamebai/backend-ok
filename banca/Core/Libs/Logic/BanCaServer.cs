...
namespace BanCa.Libs
{
    public class BanCaServer
    {
            ...
            try
            {
                if (IsDead())
                {
                    ...
                    try
                    {
                        ...
                        // Phase 5b: when the unified-wallet flag is on, collect
                        // (userId, profit, sessionId) tuples and flush them as a
                        // batch to MoneyGateway. Each entry carries its own
                        // external_ref (banca:settle:{userId}:{sessionId}:{checkpoint})
                        // so retries on the failed_settle worker are idempotent.
                        bool useUnifiedWallet = MoneyGatewayClient.IsEnabled();
                        var settleBatch = useUnifiedWallet
                            ? new List<(long userId, long profit, string sessionId)>()
                            : null;
                        for (int i = 0, n = bancaWorlds.Count; i < n; i++)
                        {
                            var world = bancaWorlds[i];
                            ...
                            for (int j = 0, m = world.players.Length; j < m; j++)
                            {
                                ...
                                if (useUnifiedWallet)
                                {
                                    settleBatch.Add((player.Id, player.Profit, "bc-revive-" + player.Id + "-" + world.Id));
                                }
                                else
                                {
                        ...
                        batch.Execute();
                        if (useUnifiedWallet && settleBatch.Count > 0)
                        {
                            try
                            {
                                await MoneyGatewayClient.BatchSettleAsync(settleBatch, "EMERGENCY_BANCA");
                            }
                            catch (Exception ex2)
                            {
                                Logger.Error("Phase5b Revive BatchSettle failed: " + ex2.Message);
                            }
                        }
                    }
                    catch (Exception ex)
                ...
                case "openTx": // transfer all cash from bc back to redis
                    {
                        ...
                        if (world != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (p != null)
                                {
                                    // Phase 5b: cross-game transfer (BanCa -> TaiXiu).
                                    // Must settle the in-memory profit to the wallet
                                    // BEFORE handing the player off, otherwise the
                                    // other game sees a stale balance. Flag off ->
                                    // legacy Redis path.
                                    if (MoneyGatewayClient.IsEnabled())
                                    {
                                        var pid = p.Id;
                                        var profit = p.Profit;
                                        var sessionId = "bc-openTx-" + pid + "-" + world.Id;
                                        var txType = profit >= 0 ? "WAGER_CREDIT_BANCA" : "WAGER_DEBIT_BANCA";
                                        TaskRunner.RunOnPool(async () =>
                                        {
                                            try { await MoneyGatewayClient.SettleAsync(pid, profit, sessionId, txType); }
                                            catch (Exception ex) { Logger.Error("Phase5b openTx settle err id=" + pid + " " + ex.Message); }
                                        });
                                    }
                                    else
                                    {
        ...
        // Phase 5b — last periodic flush timestamp (ms) and per-player
        // cumulative profit at last flush (used to detect big-bet bursts).
        private long lastPeriodicSettle;
        ...
        void _updateEngine()
        {
            ...
            // Phase 5b periodic settle: every BANCA_SETTLE_INTERVAL_MS
            // (default 5000) walk all active non-bot players and flush
            // their accumulated profit to MoneyGateway when |profit| has
            // crossed BANCA_SETTLE_THRESHOLD or BANCA_BIG_BET_THRESHOLD.
            // We do NOT touch Player.Cash. We only zero Player.Profit on a
            // successful settle so the hot path keeps the live balance.
            // Flag off -> entire block is a no-op.
            if (MoneyGatewayClient.IsEnabled())
            {
                var interval = MoneyGatewayClient.SettleIntervalMs;
                if (now - lastPeriodicSettle >= interval)
                {
                    var threshold = MoneyGatewayClient.SettleThreshold;
                    var bigBet = MoneyGatewayClient.BigBetThreshold;
                    var idleMs = MoneyGatewayClient.IdleSettleMs;
                    var sessionOnly = MoneyGatewayClient.SessionOnlySettle;
                    Logger.Info($"[trace] settle-tick now={now} interval={interval} last={lastPeriodicSettle} elapsed={now-lastPeriodicSettle} threshold={threshold} bigBet={bigBet} idleMs={idleMs} sessionOnly={sessionOnly}");
                    lastPeriodicSettle = now;
                    for (int i = 0, n = bancaWorlds.Count; i < n; i++)
                    {
                        var world = bancaWorlds[i];
                        ...
                        for (int j = 0, m = world.players.Length; j < m; j++)
                        {
                            ...
                            var profit = player.Profit;
                            if (profit == 0) continue;
                            var abs = profit < 0 ? -profit : profit;
                            var pid = player.Id;
                            // Idle = stopped shooting; force-flush so history
                            // closes the session at the silence boundary even
                            // when |profit| is below the periodic threshold.
                            var silentMs = player.LastShotMs > 0 ? now - player.LastShotMs : 0;
                            var isIdle   = idleMs > 0 && player.LastShotMs > 0 && silentMs >= idleMs;
                            // sessionOnly mode → only fire on idle so wager
                            // history rows are 1:1 with play episodes. Skip
                            // mid-session threshold flushes entirely.
                            var willFire = sessionOnly
                                ? isIdle
                                : (isIdle || abs >= threshold || abs >= bigBet);
                            Logger.Info($"[trace] settle-check player={pid} profit={profit} abs={abs} threshold={threshold} bigBet={bigBet} silentMs={silentMs} isIdle={isIdle} sessionOnly={sessionOnly} willFire={willFire}");
                            if (!willFire) continue;
                            // Idle close → emit session-end bracket then rotate
                            // SessionStartedMs so the next shoot opens a fresh
                            // session in history.
                            if (isIdle && player.SessionStartedMs > 0)
                            {
                                var durMs = now - player.SessionStartedMs;
                                Logger.Info($"[trace] session-end player={pid} world={world.Id} reason=idle durationMs={durMs} silentMs={silentMs} profit={profit}");
                                player.SessionStartedMs = 0;
                            }
                            var sessionId = isIdle
                                ? "bc-idle-" + pid + "-" + world.Id + "-" + now
                                : "bc-tick-" + pid + "-" + world.Id;
                            var txType = profit >= 0 ? "WAGER_CREDIT_BANCA" : "WAGER_DEBIT_BANCA";
                            // Settle on a background pool so the engine
                            // tick is not blocked by HTTP latency. Player
                            // Profit is zeroed only after the await
                            // returns ok — under racing shots the new
                            // accumulated profit since dispatch is
                            // preserved (delta - profit).
                            player.Profit = 0; // reserve: any new shots after dispatch accumulate fresh
                            Logger.Info($"[trace] settle-call player={pid} profit={profit} sid={sessionId}");
                            TaskRunner.RunOnPool(async () =>
                            {
                                try
                                {
                                    var ok = await MoneyGatewayClient.SettleAsync(pid, profit, sessionId, txType);
                                    Logger.Info($"[trace] settle-result player={pid} ok={ok} profit_zeroed_if_ok=true");
                                    if (ok)
                                    {
                                        // One bc_log row per play episode = source of truth for
                                        // c=303 (player) and c=9843 (agency) wallet history.
                                        try
                                        {
                                            BanCa.Sql.SqlLogger.LogSessionEnd(
                                                world.Id, world.TableBlindIndex, pid,
                                                player.Cash, profit, world.ServerId,
                                                sessionId, isIdle ? "idle" : "tick");
                                        }
                                        catch (Exception logEx)
                                        {
                                            Logger.Warning("LogSessionEnd failed pid=" + pid + " err=" + logEx.Message);
                                        }
                                    }
                                    else
                                    {
                                        // refund the in-memory profit so the next tick re-flushes
                                        try { player.Profit += profit; }
                                        catch (Exception) { /* player may have left */ }
                                    }
                                }
                                catch (Exception ex)
                                {
                                    ...
                                    Logger.Error("Phase5b periodic settle err id=" + pid + " " + ex.Message);
                                }
                            });
                        }
                    }
