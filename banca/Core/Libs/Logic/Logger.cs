...
public class Logger
{
    ...
    // Warning overloads — ILogger has no Warn channel so forward to Info
    // with a "WARN:" prefix (same category SUN-1054's LogBetCommission +
    // EpicApi recoverable-failure callsites expect).
    public static void Warning(object msg)
    {
        if (instanceLogger != null)
            instanceLogger.Info("WARN: " + msg);
    }
    public static void Warning(object msg, object arg1)
    {
        if (instanceLogger != null)
            instanceLogger.Info("WARN: " + string.Format(msg.ToString(), arg1));
    }
    public static void Warning(object msg, params object[] args)
    {
        if (instanceLogger != null)
            instanceLogger.Info("WARN: " + string.Format(msg.ToString(), args));
    }
