...
namespace BanCa.Libs
{
    public class Player
    {
        ...
        // Wall-clock ms of the most recent SHOOT command for this player.
        // Used by BanCaServer settle-tick to detect idle sessions and force
        // a profit flush so wager history closes cleanly when the player
        // stops shooting but remains in the room.
        internal long LastShotMs { get; set; }

        // Wall-clock ms when this play "session" opened (first SHOOT after
        // login or after an idle-close). Zero means no active session.
        // BanCaServer rotates it back to 0 after an idle-flush or
        // RemovePlayer settle so history brackets stay aligned.
        internal long SessionStartedMs { get; set; }
