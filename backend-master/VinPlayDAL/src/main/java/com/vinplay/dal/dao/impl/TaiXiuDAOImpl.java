...
implements TaiXiuDAO {
    ...
    public List<TransactionTaiXiu> getLichSuGiaoDich(String nickname, int number, int moneyType) throws SQLException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");
             CallableStatement call = conn.prepareCall("CALL tx_get_lich_su_giao_dich(?,?,?)")) {
            call.setString(1, nickname);
            call.setInt(2, number);
            call.setByte(3, (byte)moneyType);
    ...
    public List<TopWin> getTopTaiXiu(int moneyType) throws SQLException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");
             CallableStatement call = conn.prepareCall("CALL tx_get_top_win(?)")) {
    ...
    public int countLichSuGiaoDichTX(String nickname, int moneyType) throws SQLException {
        int totalRecords = -1;
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");
             CallableStatement call = conn.prepareCall("CALL tx_count_lich_su_giao_dich(?, ?,?)")) {
            call.setString(1, nickname);
            call.setByte(2, (byte)moneyType);
    ...
    public List<TransactionTaiXiuDetail> getChiTietPhien(long referenceId, int moneyType) throws SQLException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");
             CallableStatement call = conn.prepareCall("CALL tx_get_chi_tiet_phien(?,?)")) {
            call.setLong(1, referenceId);
            call.setByte(2, (byte)moneyType);
            try (ResultSet rs = call.executeQuery()) {
                while (rs.next()) {
    ...
    public ResultTaiXiu getKetQuaPhien(long referenceId, int moneyType) throws SQLException {
        System.out.println("TAIXIUDEBUG TaiXiuDAOImpl getKetQuaPhien: " + referenceId + " " + moneyType);
        ResultTaiXiu entry = null;
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");){
            String sql = "SELECT * FROM result_tai_xiu WHERE reference_id=" + referenceId + " AND money_type=" + moneyType;
            System.out.println("TAIXIUDEBUG TaiXiuDAOImpl getKetQuaPhien: " + sql);
            PreparedStatement stmt = conn.prepareStatement(sql);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
    ...
    public List<ThanhDuTXModel> getTopThanhDuDaily(String startTime, String endTime, short type) throws SQLException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");
             CallableStatement call = conn.prepareCall("CALL tx_get_top_thanh_du(?, ?, ?)")) {
            call.setString(1, startTime);
            call.setString(2, endTime);
            call.setByte(3, (byte)type);
            try (ResultSet rs = call.executeQuery()) {
                while (rs.next()) {
    ...
    private List<XepHangRLTLModel> getBangXepHangTLRL(String collectionName) {
        ...
        MongoDatabase db = MongoDBConnectionFactory.getDB();
        AggregateIterable iterable = db.getCollection(collectionName).aggregate(Arrays.asList(new Document[]{new Document("$group", new Document("_id", "$user_name").append("total", new Document("$sum", "$money"))), new Document("$sort", new Document("total", -1)), new Document("$limit", 10)}));
        iterable.forEach((Block)new Block<Document>(){
    ...
    private long getTienTLRL(String username, String collectionName) {
        ...
        AggregateIterable iterable = db.getCollection(collectionName).aggregate(Arrays.asList(new Document[]{new Document("$match", condition), new Document("$group", new Document("_id", "null").append("total", new Document("$sum", "$money")))}));
