...
import com.gamebase.dao.MailBoxDao;
...
implements MailBoxDao {
    ...
    public List<MailBoxResponse> listMailBox(String nickName, int page) {
        ...
        objsort.put("_id", -1);
        FindIterable iterable = db.getCollection("mail_box").find((Bson)conditions).skip(num_start).limit(5).sort((Bson)objsort);
    ...
    public int updateStatusMailBox(String mailId) {
        ...
        colmail.updateMany((Bson)new Document("mail_id", mailId), (Bson)new Document("$set", new Document("status", 1)));
        return 0;
    }
