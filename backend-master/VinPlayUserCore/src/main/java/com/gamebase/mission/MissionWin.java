...
public class MissionWin
extends BaseMissionProcess
implements MissionProcess {
    private static final Logger logger = Logger.getLogger((String)"base_game");

    public MissionWin(EventMissionService eventMissionService, Mission mission, UserMissionService userMissionService) {
    ...
    @Override
    public boolean rewardMission(String username) throws Exception {
        long money = this.mission.getReward();
        return this.baseRewardMission(username, money);
    }
