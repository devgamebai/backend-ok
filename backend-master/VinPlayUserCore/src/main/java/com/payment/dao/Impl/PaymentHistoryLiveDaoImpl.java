...
import com.payment.dao.PaymentHistoryLiveDao;
...
public class PaymentHistoryLiveDaoImpl
implements PaymentHistoryLiveDao {
    @Override
    public List<PaymentHistoryEntity> getAllByNickname(String nickname, int page, int maxItem) throws SQLException {
        ...
        String sql = "SELECT * FROM (SELECT 'bank' as type, fid, key_id as request_id, nick_name, NULL as serial, NULL as code, cash, cash_real, status, text, day, month, year, time, number FROM history_bank_live WHERE nick_name = ? UNION ALL SELECT 'topup' as type, fid, key_id as request_id, nick_name, serial, code, cash, cash_real, status, text, day, month, year, time, NULL as number FROM topup_live WHERE nick_name = ? ) as combined ORDER BY time DESC LIMIT ? OFFSET ?";
    ...
    public int getTotalCount(String nickname) throws SQLException {
        String sql = "SELECT COUNT(*) as total FROM (SELECT key_id FROM history_bank_live WHERE nick_name = ? UNION ALL SELECT key_id FROM topup_live WHERE nick_name = ?) as combined";
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpoolname");
             PreparedStatement stm = conn.prepareStatement(sql);){
            stm.setString(1, nickname);
            stm.setString(2, nickname);
