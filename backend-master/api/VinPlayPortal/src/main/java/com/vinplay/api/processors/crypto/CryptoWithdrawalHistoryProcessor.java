...
/**
 * c=3023 — Get crypto withdrawal history for the authenticated user.
 */
public class CryptoWithdrawalHistoryProcessor implements BaseProcessor<HttpServletRequest, String> {
        ...
        try {
            ...
            try {
                ...
                ps = conn.prepareStatement(
                        "SELECT id, to_address, amount_krw, amount_usdt, status, created_at, processed_at " +
                        "FROM crypto_withdrawals WHERE user_id = ? ORDER BY id DESC LIMIT ? OFFSET ?");
                ps.setLong(1, userId);
                ps.setInt(2, limit);
                ps.setInt(3, offset);
                rs = ps.executeQuery();
            ...
            logger.error("CryptoWithdrawalHistoryProcessor error", e);
            response.put("success", false);
            response.put("errorCode", "1001");
        }
        return response.toString();
