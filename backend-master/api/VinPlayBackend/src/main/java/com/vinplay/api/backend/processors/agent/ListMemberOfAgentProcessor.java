...
/**
 * c=8830 -- List Members of an agent (Chỉ lấy tài khoản người chơi thường, dai_ly = 0).
 * Params: rc (nguoi dang nhap), nn (nickname filter), ft, et, p, l
 */
public class ListMemberOfAgentProcessor implements BaseProcessor<HttpServletRequest, String> {
    ...
    public String execute(Param<HttpServletRequest> param) {
        ...
        try {
            ...
            try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpoolname")) {
                ...
                if (currentLevel < 1) {
                    // Neu khong phai Dai ly (dai_ly = 0 hoac khong ton tai), tra ve rong
                ...
                // Build Where - Quan trong la AND dai_ly = 0
                StringBuilder where = new StringBuilder(" WHERE parrentUser = ? AND dai_ly = 0");
                ...
                String dataSql = "SELECT id, user_name, nick_name, dai_ly, vin, 0 AS xu, 0 AS recharge_money, " + // SUN-13xx: xu dropped, kept alias for response parity
                        "t_nap, t_rut, parrentUser, mobile, create_time, last_login, status " +
                ...
                try (PreparedStatement psData = conn.prepareStatement(dataSql)) {
                    ...
                    try (ResultSet rs = psData.executeQuery()) {
                        while (rs.next()) {
                            ...
                            item.put("parrentUser", rs.getString("parrentUser") != null ? rs.getString("parrentUser") : "");
            ...
            logger.error("ListMemberOfAgentProcessor error", e);
            response.put("success", false);
