...
/**
 * c=9924 — Admin thu hồi tiền từ Credit Wallet của agent.
 * Path: /api_backend (yêu cầu admin access token)
 * Params:
 *   at  = admin access token
 *   nn  = agent nickname (đại lý bị thu hồi điểm)
 *   am  = amount (1 to 10,000,000)
 *   nt  = note (optional)
 *
 * Không cần duyệt, không cần OTP — admin action trực tiếp.
 */
public class AdminCreditWalletRevokeProcessor implements BaseProcessor<HttpServletRequest, String> {
    ...
    public String execute(Param<HttpServletRequest> param) {
        ...
        try {
            ...
            CreditWalletService.CreditResult result =
                    CreditWalletService.adminRevoke(agent.id, agent.nickname, amount, adminNick, note);
            ...
            logger.error("AdminCreditWalletRevokeProcessor error", e);
