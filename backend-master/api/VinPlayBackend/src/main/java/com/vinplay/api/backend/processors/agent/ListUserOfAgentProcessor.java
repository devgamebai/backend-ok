...
/**
 * c=8827 -- List users of a specific agent.
 * Params: rc (referral code / agent nickname), nn (user nickname filter), ft, et, p
 */
    ...
    public String execute(Param<HttpServletRequest> param) {
        ...
        try {
            ...
            try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpoolname")) {
                // Count
                int total = 0;
                try (PreparedStatement ps = conn.prepareStatement("SELECT COUNT(*) FROM users" + where)) {
                ...
                // Data
                String dataSql = "SELECT id, user_name, nick_name, vin, 0 AS xu, 0 AS recharge_money, " + // SUN-13xx: xu dropped, kept alias for response parity
                        "t_nap, t_rut, parrentUser, create_time, last_login, status " +
            ...
            logger.error("ListUserOfAgentProcessor error", e);
