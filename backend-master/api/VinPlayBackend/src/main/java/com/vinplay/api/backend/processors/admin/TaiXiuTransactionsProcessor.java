...
public class TaiXiuTransactionsProcessor implements BaseProcessor<HttpServletRequest, String> {
    ...
    public String execute(Param<HttpServletRequest> param) {
        ...
        try {
            ...
            if (mt != null && !mt.isEmpty()) {
                where.append(" AND money_type = ?");
                params2.add(mt);
            }

            try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame")) {
                int total = 0;
                try (PreparedStatement countPs = conn.prepareStatement("SELECT COUNT(*) FROM transaction_tai_xiu" + where.toString())) {
                ...
                String sql = "SELECT * FROM transaction_tai_xiu" + where.toString() + " ORDER BY id DESC LIMIT ?,?";
            ...
            logger.error("TaiXiuTransactionsProcessor error", e);
