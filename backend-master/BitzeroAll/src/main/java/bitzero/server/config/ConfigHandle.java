...
public class ConfigHandle {
    ...
    private ConfigHandle() {
        try {
            ...
            String clusterPath = VBeePath.basePath.concat("conf" + File.separator + "cluster.properties");
            File clusterFile = new File(clusterPath);
            if (clusterFile.exists()) {
                this.props.load(new FileInputStream(clusterFile));
            } else {
                // Missing cluster.properties should not crash the whole server.
                // Other modules may provide their own defaults at runtime.
                DebugConsole.log.warn("Missing config file: " + clusterFile.getAbsolutePath());
            }
            this.propsByGames = new ConcurrentHashMap();
            String gamesProp = this.props.getProperty("games");
            if (gamesProp != null && !gamesProp.trim().isEmpty()) {
                String[] games = gamesProp.split(";");
    ...
    public Long getLong(String name) {
        ...
        if (result == null) {
            result = new Long(0);
            try {
                String raw = this.props != null ? this.props.getProperty(name) : null;
                if (raw == null || raw.trim().isEmpty()) {
                    result = 0L;
                } else {
                    result = Long.parseLong(raw.trim());
                }
            }
            catch (Exception e) {
                e.printStackTrace();
