...
public final class DefaultSessionManager implements ISessionManager {
     ...
     public void shutDownLocalSessions() {
          synchronized(this.localSessions) {
               ...
               while(it.hasNext()) {
                    ISession session = ((it.next() instanceof bitzero.engine.sessions.ISession) ? (bitzero.engine.sessions.ISession)it.next() : ((bitzero.server.entities.User)it.next()).getSession());
                    it.remove();
