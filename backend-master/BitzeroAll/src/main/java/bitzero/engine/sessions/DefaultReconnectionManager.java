...
public final class DefaultReconnectionManager implements IService, IReconnectionManager {
     ...
     private void applySessionCleaning() {
          if (this.frozenSessionsByHash.size() > 0) {
               ...
               while(iter.hasNext()) {
                    ISession session = ((iter.next() instanceof bitzero.engine.sessions.ISession) ? (bitzero.engine.sessions.ISession)iter.next() : ((bitzero.server.entities.User)iter.next()).getSession());
