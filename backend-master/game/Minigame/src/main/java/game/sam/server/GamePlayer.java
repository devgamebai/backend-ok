...
public class GamePlayer {
    ...
    public int takeTurn(boolean baomot, byte[] cards, Round round) {
        ...
        if (playCard.BO == 0) {
            Debug.trace("Turn loi khong dong bo");
            return 1;
        }
        if (!this.spInfo.handCards.kiemTraBoBaiDanhRa(playCard)) {
            Debug.trace("Thoi hai, hoac khong co bo bai duoc danh ra");
            return 1;
        }
        if (prevTurn == null) {
            turn.turnType = 1;
            ...
            if (!playCard.chatDuoc(prevCards)) {
                Debug.trace("Turn loi khong chat duoc");
                Debug.trace(playCard.toString());
                Debug.trace(prevCards.toString());
                return 1;
            }
        }
    ...
    public long calculateMoneyLost(int kieuThang, long soTienNguoiThang, long moneyBet) {
        ...
        Debug.trace(this.spInfo.handCards.toString());
    ...
    public byte[] getMinCardS() {
        ...
        Debug.trace(new Card(cards[0]).toString());
        return cards;
    }
