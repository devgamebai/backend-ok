...
public class BotJackPotSlot3x3Timer
implements Runnable {
    public boolean isInitBot = false;
    public List<String> bots;
    private long timeJackPot_100 = 0L;
    ...
    public void run() {
        try {
            ...
            int time = 0;
            if (currentTime > this.timeJackPot_100) {
                time = GameConfig.getInstance().slot3x3BotConfig.randomTimeBotEat(0);
                this.timeJackPot_100 = currentTime + (long)time;
                room100.botEatJackpot("Bot_Slot3x3_time_vin_100", this.timeJackPot_100, this.bots.get(GameUtil.randomMax(this.bots.size())));
            }
            if (currentTime > this.timeJackPot_1000) {
                time = GameConfig.getInstance().slot3x3BotConfig.randomTimeBotEat(1);
                this.timeJackPot_1000 = currentTime + (long)time;
                room1000.botEatJackpot("Bot_Slot3x3_time_vin_1000", this.timeJackPot_1000, this.bots.get(GameUtil.randomMax(this.bots.size())));
            }
            if (currentTime > this.timeJackPot_10000) {
                time = GameConfig.getInstance().slot3x3BotConfig.randomTimeBotEat(2);
                this.timeJackPot_10000 = currentTime + (long)time;
                room10000.botEatJackpot("Bot_Slot3x3_time_vin_10000", this.timeJackPot_10000, this.bots.get(GameUtil.randomMax(this.bots.size())));
            }
        }
        catch (Exception e) {
