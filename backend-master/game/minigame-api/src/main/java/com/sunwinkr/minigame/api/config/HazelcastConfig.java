 ...
 * {@code getInstance()}. When the factory is already initialised (embedded
 * mode), {@code init()} creates a second client — callers that already hold
 * the old reference are unaffected; the factory field is replaced atomically.
 * The guard {@code instance != null} prevents a redundant reconnect.
 */
@Configuration
