...
import game.modules.slot.CandyModule;
import game.modules.slot.cmd.send.candy.CandyFreeDailyMsg;
import game.modules.slot.cmd.send.candy.CandyTotalFreeSpin;
import game.modules.slot.cmd.send.candy.BigWinCandyMsg;
import game.modules.slot.cmd.send.candy.ForceStopAutoPlayCandyMsg;
import game.modules.slot.cmd.send.candy.MinimizeResultCandyMsg;
import game.modules.slot.cmd.send.candy.ResultCandyMsg;
import game.modules.slot.cmd.send.candy.UpdatePotCandyMsg;
...
public class CandyRoom extends SlotRoom {
     ...
     public CandyRoom(CandyModule module, byte id, String name, short moneyType, long pot, long fund, int betValue, long initPotValue) {
          super(id, name, betValue, moneyType, pot, fund, initPotValue);
          this.module = module;
          this.moneyType = moneyType;
          this.gameName = Games.CANDY.getName();
     ...
     public void forceStopAutoPlay(User user) {
          ...
          synchronized(this.usersAuto) {
               ...
               ForceStopAutoPlayCandyMsg msg = new ForceStopAutoPlayCandyMsg();
               SlotUtils.sendMessageToUser(msg, (User)user);
          }
     }

     public ResultCandyMsg play(String username, String linesStr, boolean isBot) {
     ...
     public ResultCandyMsg playNormal(String username, String linesStr, long referenceId) {
          ...
          ResultCandyMsg msg = new ResultCandyMsg();
          long fee;
          if (lineArr.length > 0 && !linesStr.isEmpty()) {
               if (totalBetValue > 0L) {
                    if (totalBetValue <= currentMoney) {
                         fee = totalBetValue * 2L / 100L;
                         MoneyResponse moneyRes = this.userService.updateMoney(username, -totalBetValue, this.moneyTypeStr, this.gameName, "Candy", "Đặt cược Candy", fee, referenceId, TransType.START_TRANS);
                    ...
                    if (result != 3 && result != 4) {
                        if (SlotHouseEdge.shouldForceLoseByPctOnly(username, "slot_candy")) {
                    ...
                    enoughPair = true;
                                   ...
                                   if (totalPrizes > 0L) {
                                        if (result == 3) {
                                             ...
                                             totalPrizes = game.modules.slot.utils.SlotHouseEdge.maybeForceLose(totalPrizes, result, username, "slot_candy");
                                             this.fund -= totalPrizes - soTienNoHuKhongTruQuy;
                                             if (this.moneyType == 1) {
                                                  GameUtils.sendSMSToUser(username, "Chuc mung " + username + " da no hu game Candy phong " + this.betValue + ". So tien no hu: " + totalPrizes + " vin");
                                             }
                                   ...
                                   String des = "Quay Candy ";
                                   if (tienThuongX2 > 0L) {
                                        this.userService.updateMoney(username, tienThuongX2, this.moneyTypeStr, this.gameName, "Quay Candy ", "Thưởng hũ X2", 0L, (Long)null, TransType.NO_VIPPOINT);
                                   }

                                   moneyRes = this.userService.updateMoney(username, moneyExchange, this.moneyTypeStr, this.gameName, "Quay Candy ", this.buildDescription(totalBetValue, totalPrizes, result), 0L, referenceId, TransType.END_TRANS);
                                   if (moneyRes != null && moneyRes.isSuccess()) {
                                        currentMoney = moneyRes.getCurrentMoney();
                                        if (this.moneyType == 1 && moneyExchange - (long)this.betValue >= (long)BroadcastMessageServiceImpl.MIN_MONEY) {
                                             this.broadcastMsgService.putMessage(Games.CANDY.getId(), username, moneyExchange - (long)this.betValue);
                                        }
                                   }
                                   ...
                                   try {
                                        ...
                                        if (result == 3 || result == 2 || result == 4) {
                                             BigWinCandyMsg bigWinMsg = new BigWinCandyMsg();
     ...
     public ResultCandyMsg playFreeDaily(String username, long referenceId) {
                    ...
                    while(true) {
                         ...
                         if (totalPrizes <= (long)ConfigGame.getIntValue("max_prize_free_daily", 2000)) {
                              ...
                              if (!updated) {
                                   ...
                                   if (moneyExchange > 0L) {
                                        String des = "Quay Candy Free ";
                                        MoneyResponse moneyRes = this.userService.updateMoney(username, moneyExchange, this.moneyTypeStr, "CandyFree", "Quay Candy Free ", "Cược: 0, Thắng: " + totalPrizes, 0L, (Long)null, TransType.NO_VIPPOINT);
                                        if (moneyRes != null && moneyRes.isSuccess()) {
                                             currentMoney = moneyRes.getCurrentMoney();
                                        }
                                   }
     ...
     public ResultCandyMsg playFree(String username, String linesStr, String itemsWild, int ratio, long referenceId) {
          long startTime = System.currentTimeMillis();
          String currentTimeStr = DateTimeUtils.getCurrentTime();
          short result = 0;
          String[] lineArr = linesStr.split(",");
          ...
          while(true) {
               ...
               long moneyExchange = totalPrizes - 0L;
               String des = "Candy - Free";
               MoneyResponse moneyRes = this.userService.updateMoney(username, moneyExchange, this.moneyTypeStr, this.gameName, "Candy - Free", this.buildDescription(0L, totalPrizes, result), 0L, (Long)null, TransType.VIPPOINT);
               if (moneyRes != null && moneyRes.isSuccess()) {
                    currentMoney = moneyRes.getCurrentMoney();
               ...
               if (freeSpin.getNum() == 0) {
                    CandyTotalFreeSpin totalFreeSpinMsg = new CandyTotalFreeSpin();
     ...
     public short play(User user, String linesStr) {
          ...
          ResultCandyMsg msg = null;
          CandyFreeDailyMsg freeDailyMsg = new CandyFreeDailyMsg();
          if (numFree > 0) {
          ...
          if (this.isUserMinimize(user)) {
               MinimizeResultCandyMsg miniMsg = new MinimizeResultCandyMsg();
     ...
     private void savePot() {
          ...
          if (currentTime - this.lastTimeUpdatePotToRoom >= 3000L) {
               ...
               UpdatePotCandyMsg msg = new UpdatePotCandyMsg();
               msg.value = this.pot;
