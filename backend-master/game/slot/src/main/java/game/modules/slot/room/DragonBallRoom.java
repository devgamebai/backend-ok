...
import game.modules.slot.DragonBallModule;
import game.modules.slot.cmd.send.dragonball.*;
...
/**
 * DragonBall slot room — uses existing Slot3x3 engine from Minigame.jar.
 * 3x3 matrix, 6 symbols (0-5), 3 horizontal paylines, 20 client lines.
 */
public class DragonBallRoom extends SlotRoom {
    ...
    public DragonBallRoom(DragonBallModule module, byte id, String name, short moneyType, long pot, long fund, int betValue,
        ...
        this.gameID = Games.DRAGONBALL.getId() + "";
    ...
    protected final class CheckResetPotTask implements Runnable {
        public void run() {
            try {
                com.vinplay.vbee.common.utils.GameHealthServer.tick(); DragonBallRoom.this.checkResetPot(); } catch (Throwable e) { e.printStackTrace(); }
        }
    }
    ...
    public void forceStopAutoPlay(User user) {
        ...
        synchronized (map) {
            ...
            ForceStopAutoPlayDragonBallMsg msg = new ForceStopAutoPlayDragonBallMsg();
            SlotUtils.sendMessageToUser((BaseMsg) msg, user);
        }
    }

    public ResultDragonBallMsg play(String username, String linesStr) {
        ...
        try {
            ...
            Debug.trace("error get free spin dragonball");
        }
    ...
    private void broadcastBigWin(String username, byte result, long totalPrizes) {
        BigWinDragonBallMsg bigWinMsg = new BigWinDragonBallMsg();
    ...
    public ResultDragonBallMsg playNormal(String username, String linesStr, long referenceId) {
        String currentTimeStr = DateTimeUtils.getCurrentTime();
        short result = 0;
        ...
        ResultDragonBallMsg msg = new ResultDragonBallMsg();
        int[] clientLines = this.parseSelectedLines(linesStr);

        if (clientLines == null || clientLines.length == 0) {
            Debug.trace("dragonball invalid lines playNormal username=" + username + " lines=" + linesStr);
            msg.result = (byte) 101;
        ...
        if (clientLines.length > 0 && totalBetValue > 0L) {
            if (totalBetValue <= moneyAvailable) {
                ...
                if (moneyRes != null && moneyRes.isSuccess()) {
                    ...
                    boolean forceLose = SlotHouseEdge.shouldForceLoseWithPct(this.fund, this.fundJackPot, totalBetValue, this.betValue, username, "slot_dragonball");
                    ...
                    int[][] forcedIds = game.modules.slot.utils.SlotForceResultHelper.checkAndConsume("DRAGONBALL", username, 3, 3);
                    if (forcedIds != null) {
                    ...
                    if (totalPrizes != 0 && !u.isBot()) {
                        if ((moneyRes = this.userService.updateMoney(username, totalPrizes, this.moneyTypeStr, this.gameName,
                            ...
                            if (this.moneyType == 1 && moneyExchange >= (long) BroadcastMessageServiceImpl.MIN_MONEY) {
                                this.broadcastMsgService.putMessage(Games.DRAGONBALL.getId(), username, moneyExchange - totalBetValue);
                            }
                        }
                    }
        ...
        msg.currentMoney = currentMoney;
        Debug.trace("Normal Spin dragonball 3x3", "totalWin = " + msg.prize, "  pot = " + this.pot, " fund =" + this.fund);
        return msg;
    }

    public ResultDragonBallMsg playFree(String username, String linesStr, int ratio, long referenceId) {
        String currentTimeStr = DateTimeUtils.getCurrentTime();
        short result = 0;
        long currentMoney = this.userService.getCurrentMoneyUserCache(username, this.moneyTypeStr);
        ResultDragonBallMsg msg = new ResultDragonBallMsg();
    ...
    public short play(User user, String linesStr) {
        ResultDragonBallMsg msg = this.play(user.getName(), linesStr);
    ...
    public void updatePot(User user) {
        UpdatePotDragonBallMsg msg = new UpdatePotDragonBallMsg();
        msg.value = this.pot;
    ...
    protected void playListAuto(List users) {
        for (Object obj : users) {
            ...
            try {
                ...
                Logger.getLogger(DragonBallRoom.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        users.clear();
    }
    ...
    private class PlayListAutoUserTask implements Runnable {
        ...
        @Override
        public void run() {
            try { DragonBallRoom.this.playListAuto(this.users); } catch (Throwable e) { Debug.trace("AutoPlay dragonball error: " + e.getMessage()); }
        }
    }

    protected final class GameLoopTask implements Runnable {
        public void run() {
            try { DragonBallRoom.this.gameLoop(); } catch (Throwable e) { e.printStackTrace(); }
        }
    }
    ...
    public boolean joinRoom(User user) {
        ...
        DragonBallFreeDailyMsg freeDailyMsg = new DragonBallFreeDailyMsg();
