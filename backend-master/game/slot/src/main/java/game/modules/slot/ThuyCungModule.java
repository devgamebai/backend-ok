...
import game.modules.slot.cmd.rev.thuycung.AutoPlayThuyCungCmd;
import game.modules.slot.cmd.rev.thuycung.ChangeRoomThuyCungCmd;
import game.modules.slot.cmd.rev.thuycung.PlayThuyCungCmd;
import game.modules.slot.cmd.rev.thuycung.SubscribeThuyCungCmd;
import game.modules.slot.cmd.rev.thuycung.UnSubscribeThuyCungCmd;
import game.modules.slot.cmd.rev.khobau.MinimizeKhoBauCmd;
import game.modules.slot.cmd.send.thuycung.ThuyCungInfoMsg;
import game.modules.slot.entities.BotMinigame;
import game.modules.slot.room.ThuyCungRoom;
...
public class ThuyCungModule extends SlotModule {
     ...
     public void init() {
          ...
          try {
               ...
               Debug.trace(new Object[]{"Init ThuyCung error ", var7});
               initPotValues[0] = defaultPots[0]; initPotValues[1] = defaultPots[1]; initPotValues[2] = defaultPots[2];
          }

          this.rooms.put(this.gameName + "_vin_100", new ThuyCungRoom(this, (byte)0, this.gameName + "_vin_100", (short)1, this.pots[0], funds[0], 100, (long)initPotValues[0]));
          this.rooms.put(this.gameName + "_vin_1000", new ThuyCungRoom(this, (byte)1, this.gameName + "_vin_1000", (short)1, this.pots[1], funds[1], 1000, (long)initPotValues[1]));
          this.rooms.put(this.gameName + "_vin_10000", new ThuyCungRoom(this, (byte)2, this.gameName + "_vin_10000", (short)1, this.pots[2], funds[2], 10000, (long)initPotValues[2]));
          Debug.trace("INIT ThuyCung DONE");
          this.getParentExtension().addEventListener(BZEventType.USER_DISCONNECT, this);
          this.referenceId = this.slotService.getLastReferenceId(this.gameName);
          Debug.trace("START ThuyCung REFERENCE ID= " + this.referenceId);
     ...
     private void userDis(User user) {
          ThuyCungRoom room = (ThuyCungRoom)user.getProperty("MGROOM_" + this.gameName + "_INFO");
          if (room != null) {
               room.quitRoom(user);
     ...
     protected void subScribe(User user, DataCmd dataCmd) {
          SubscribeThuyCungCmd cmd = new SubscribeThuyCungCmd(dataCmd);
          ThuyCungRoom room = (ThuyCungRoom)this.getRoom(cmd.roomId);
          if (room != null) {
               room.joinRoom(user);
               room.userMaximize(user);
               room.updatePot(user);
               this.updateThuyCungInfo(user, room);
          } else {
     ...
     private void updateThuyCungInfo(User user, ThuyCungRoom room) {
          ThuyCungInfoMsg msg = new ThuyCungInfoMsg();
     ...
     protected void unSubScribe(User user, DataCmd dataCmd) {
          UnSubscribeThuyCungCmd cmd = new UnSubscribeThuyCungCmd(dataCmd);
          ThuyCungRoom room = (ThuyCungRoom)this.getRoom(cmd.roomId);
          if (room != null) {
               room.stopAutoPlay(user);
               room.quitRoom(user);
     ...
     protected void changeRoom(User user, DataCmd dataCmd) {
          ChangeRoomThuyCungCmd cmd = new ChangeRoomThuyCungCmd(dataCmd);
          ThuyCungRoom roomLeaved = (ThuyCungRoom)this.getRoom(cmd.roomLeavedId);
          ThuyCungRoom roomJoined = (ThuyCungRoom)this.getRoom(cmd.roomJoinedId);
          if (roomLeaved != null && roomJoined != null) {
               ...
               this.updateThuyCungInfo(user, roomJoined);
          } else {
     ...
     private void play(User user, DataCmd dataCmd) {
          PlayThuyCungCmd cmd = new PlayThuyCungCmd(dataCmd);
          ThuyCungRoom room = (ThuyCungRoom)user.getProperty("MGROOM_" + this.gameName + "_INFO");
          if (room != null) {
     ...
     private void autoPlay(User user, DataCmd dataCMD) {
          AutoPlayThuyCungCmd cmd = new AutoPlayThuyCungCmd(dataCMD);
          ThuyCungRoom room = (ThuyCungRoom)user.getProperty("MGROOM_" + this.gameName + "_INFO");
          if (room != null) {
               if (cmd.autoPlay == 1) {
          ...
          if (this.countBot100 >= this.getCountTimeBot(this.gameName + "_bot_100")) {
               if (this.countBot100 == this.getCountTimeBot(this.gameName + "_bot_100")) {
                    ...
                    while(var2.hasNext()) {
                         ...
                         if (bot != null) {
                              room = (ThuyCungRoom)this.rooms.get(this.gameName + "_vin_100");
          ...
          if (this.countBot1000 >= this.getCountTimeBot(this.gameName + "_bot_1000")) {
               if (this.countBot1000 == this.getCountTimeBot(this.gameName + "_bot_1000")) {
                    ...
                    while(var2.hasNext()) {
                         bot = (String)var2.next();
                         if (bot != null) {
                              room = (ThuyCungRoom)this.rooms.get(this.gameName + "_vin_1000");
                              room.play(bot, this.fullLines, true);
                         }
                    }
               }
          ...
          if (this.countBot10000 >= this.getCountTimeBot(this.gameName + "_bot_10000")) {
               if (this.countBot10000 == this.getCountTimeBot(this.gameName + "_bot_10000")) {
                    ...
                    while(var2.hasNext()) {
                         bot = (String)var2.next();
                         if (bot != null) {
                              room = (ThuyCungRoom)this.rooms.get(this.gameName + "_vin_10000");
                              room.play(bot, this.fullLines, true);
                         }
                    }
               }
