...
import game.modules.slot.cmd.rev.candy.AutoPlayCandyCmd;
import game.modules.slot.cmd.rev.candy.ChangeRoomCandyCmd;
import game.modules.slot.cmd.rev.candy.PlayCandyCmd;
import game.modules.slot.cmd.rev.candy.SubscribeCandyCmd;
import game.modules.slot.cmd.rev.candy.UnSubscribeCandyCmd;
import game.modules.slot.cmd.rev.khobau.MinimizeKhoBauCmd;
import game.modules.slot.cmd.send.candy.CandyInfoMsg;
import game.modules.slot.entities.BotMinigame;
import game.modules.slot.room.CandyRoom;
...
public class CandyModule extends SlotModule {
     ...
     public void init() {
          ...
          this.rooms.put(this.gameName + "_vin_100", new CandyRoom(this, (byte)0, this.gameName + "_vin_100", (short)1, this.pots[0], funds[0], 100, (long)initPotValues[0]));
          this.rooms.put(this.gameName + "_vin_1000", new CandyRoom(this, (byte)1, this.gameName + "_vin_1000", (short)1, this.pots[1], funds[1], 1000, (long)initPotValues[1]));
          this.rooms.put(this.gameName + "_vin_10000", new CandyRoom(this, (byte)2, this.gameName + "_vin_10000", (short)1, this.pots[2], funds[2], 10000, (long)initPotValues[2]));
     ...
     private void userDis(User user) {
          CandyRoom room = (CandyRoom)user.getProperty("MGROOM_" + this.gameName + "_INFO");
          if (room != null) {
               room.quitRoom(user);
     ...
     protected void subScribe(User user, DataCmd dataCmd) {
          SubscribeCandyCmd cmd = new SubscribeCandyCmd(dataCmd);
          CandyRoom room = (CandyRoom)this.getRoom(cmd.roomId);
          if (room != null) {
               room.joinRoom(user);
               room.userMaximize(user);
               room.updatePot(user);
               this.updateCandyInfo(user, room);
          } else {
     ...
     private void updateCandyInfo(User user, CandyRoom room) {
          CandyInfoMsg msg = new CandyInfoMsg();
     ...
     protected void unSubScribe(User user, DataCmd dataCmd) {
          UnSubscribeCandyCmd cmd = new UnSubscribeCandyCmd(dataCmd);
          CandyRoom room = (CandyRoom)this.getRoom(cmd.roomId);
          if (room != null) {
               room.stopAutoPlay(user);
               room.quitRoom(user);
     ...
     protected void changeRoom(User user, DataCmd dataCmd) {
          ChangeRoomCandyCmd cmd = new ChangeRoomCandyCmd(dataCmd);
          CandyRoom roomLeaved = (CandyRoom)this.getRoom(cmd.roomLeavedId);
          CandyRoom roomJoined = (CandyRoom)this.getRoom(cmd.roomJoinedId);
          if (roomLeaved != null && roomJoined != null) {
               ...
               this.updateCandyInfo(user, roomJoined);
          } else {
     ...
     private void play(User user, DataCmd dataCmd) {
          PlayCandyCmd cmd = new PlayCandyCmd(dataCmd);
          CandyRoom room = (CandyRoom)user.getProperty("MGROOM_" + this.gameName + "_INFO");
          if (room != null) {
     ...
     private void autoPlay(User user, DataCmd dataCMD) {
          AutoPlayCandyCmd cmd = new AutoPlayCandyCmd(dataCMD);
          CandyRoom room = (CandyRoom)user.getProperty("MGROOM_" + this.gameName + "_INFO");
          if (room != null) {
               if (cmd.autoPlay == 1) {
          ...
          if (this.countBot100 >= this.getCountTimeBot(this.gameName + "_bot_100")) {
               if (this.countBot100 == this.getCountTimeBot(this.gameName + "_bot_100")) {
                    ...
                    while(var2.hasNext()) {
                         ...
                         if (bot != null) {
                              room = (CandyRoom)this.rooms.get(this.gameName + "_vin_100");
          ...
          if (this.countBot1000 >= this.getCountTimeBot(this.gameName + "_bot_1000")) {
               if (this.countBot1000 == this.getCountTimeBot(this.gameName + "_bot_1000")) {
                    ...
                    while(var2.hasNext()) {
                         bot = (String)var2.next();
                         if (bot != null) {
                              room = (CandyRoom)this.rooms.get(this.gameName + "_vin_1000");
                              room.play(bot, this.fullLines, true);
                         }
                    }
               }
          ...
          if (this.countBot10000 >= this.getCountTimeBot(this.gameName + "_bot_10000")) {
               if (this.countBot10000 == this.getCountTimeBot(this.gameName + "_bot_10000")) {
                    ...
                    while(var2.hasNext()) {
                         bot = (String)var2.next();
                         if (bot != null) {
                              room = (CandyRoom)this.rooms.get(this.gameName + "_vin_10000");
                              room.play(bot, this.fullLines, true);
                         }
                    }
               }
