...
public class ChiemTinhRoom extends SlotRoom {
    ...
    private ThreadPoolExecutor executor = (ThreadPoolExecutor) Executors.newFixedThreadPool(8); // Reduced from 10 to 8
    private String gn;
...
//                        Debug.trace(" get user value maxiumWin  ", userValue, maxiumWin);
                        ...
                        // Check for admin-forced result (3x5)
                        int[][] forcedIds = game.modules.slot.utils.SlotForceResultHelper.checkAndConsume("CHIEMTINH", username, 3, 5);
                        if (forcedIds != null) {
...
//                        slot11IconWildLienTucTableInfo.calculate(rowIndex);
                        ...
                        if(!isUserBigWin) {
                            ...
                            totalPrizes = game.modules.slot.utils.SlotHouseEdge.maybeForceLose(totalPrizes, result, username, "slot_chiemtinh");
...
//                                    this.fundJackPot -= this.initPotValue;
                        ...
                        if (countFreeSpin > 0) {
                            msg.freeSpin = 1;
                            int soLuot = this.setFreeSpin(username, linesStr, countFreeSpin);
                            msg.ratio = (byte) countFreeSpin;
                            msg.currentNumberFreeSpin = (byte) soLuot;
                        } else {
                            msg.freeSpin = 0;
                        }
                        try {
...
// [DISABLED-MR28]                         if (SlotHouseEdge.shouldForceLoseByPctOnly(username, "slot_chiemtinh")) {
// [DISABLED-MR28]                             
...
//        Debug.trace(" get user value maxiumWin  ", userValue, maxiumWin);
        ...
        if (countFreeSpin > 0) {
            ...
            int soLuot = this.setFreeSpin(username, linesStr, countFreeSpin);
            msg.ratio = (byte) countFreeSpin;
            msg.currentNumberFreeSpin = (byte) (soLuot + ratio - 1);
        } else {
            msg.freeSpin = 0;
            msg.ratio = 0;
