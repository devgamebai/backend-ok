...
import game.modules.slot.ThanBaiModule;
import game.modules.slot.cmd.send.thanbai.ThanBaiFreeDailyMsg;
import game.modules.slot.cmd.send.thanbai.ThanBaiTotalFreeSpin;
import game.modules.slot.cmd.send.thanbai.BigWinThanBaiMsg;
import game.modules.slot.cmd.send.thanbai.ForceStopAutoPlayThanBaiMsg;
import game.modules.slot.cmd.send.thanbai.MinimizeResultThanBaiMsg;
import game.modules.slot.cmd.send.thanbai.ResultThanBaiMsg;
import game.modules.slot.cmd.send.thanbai.UpdatePotThanBaiMsg;
...
public class ThanBaiRoom extends SlotRoom {
     ...
     public ThanBaiRoom(ThanBaiModule module, byte id, String name, short moneyType, long pot, long fund, int betValue, long initPotValue) {
     ...
     public void forceStopAutoPlay(User user) {
          ...
          synchronized(this.usersAuto) {
               ...
               ForceStopAutoPlayThanBaiMsg msg = new ForceStopAutoPlayThanBaiMsg();
               SlotUtils.sendMessageToUser(msg, (User)user);
          }
     }

     public ResultThanBaiMsg play(String username, String linesStr, boolean isBot) {
     ...
     public ResultThanBaiMsg playNormal(String username, String linesStr, long referenceId) {
          ...
          ResultThanBaiMsg msg = new ResultThanBaiMsg();
          long fee;
          if (lineArr.length > 0 && !linesStr.isEmpty()) {
               if (totalBetValue > 0L) {
                    if (totalBetValue <= currentMoney) {
                         fee = totalBetValue * 2L / 100L;
                         MoneyResponse moneyRes = this.userService.updateMoney(username, -totalBetValue, this.moneyTypeStr, this.gameName, "ThanBai", "Đặt cược ThanBai", fee, referenceId, TransType.START_TRANS);
                    ...
                    // [SUN-589 Phase 3c] RTP Engine — exempt jackpot (result==3||4)
                    if (result != 3 && result != 4) {
                        if (SlotHouseEdge.shouldForceLoseByPctOnly(username, "slot_thanbai")) {
                    ...
                    enoughPair = true;
                                   ...
                                   if (totalPrizes > 0L) {
                                        if (result == 3) {
                                             ...
                                             totalPrizes = game.modules.slot.utils.SlotHouseEdge.maybeForceLose(totalPrizes, result, username, "slot_thanbai");
                                             this.fund -= totalPrizes - soTienNoHuKhongTruQuy;
                                             if (this.moneyType == 1) {
                                                  GameUtils.sendSMSToUser(username, "Chuc mung " + username + " da no hu game ThanBai phong " + this.betValue + ". So tien no hu: " + totalPrizes + " vin");
                                             }
                                   ...
                                   String des = "Quay ThanBai ";
                                   if (tienThuongX2 > 0L) {
                                        this.userService.updateMoney(username, tienThuongX2, this.moneyTypeStr, this.gameName, "Quay ThanBai ", "Thưởng hũ X2", 0L, (Long)null, TransType.NO_VIPPOINT);
                                   }

                                   moneyRes = this.userService.updateMoney(username, moneyExchange, this.moneyTypeStr, this.gameName, "Quay ThanBai ", this.buildDescription(totalBetValue, totalPrizes, result), 0L, referenceId, TransType.END_TRANS);
                                   if (moneyRes != null && moneyRes.isSuccess()) {
                                        currentMoney = moneyRes.getCurrentMoney();
                                        if (this.moneyType == 1 && moneyExchange - (long)this.betValue >= (long)BroadcastMessageServiceImpl.MIN_MONEY) {
                                             this.broadcastMsgService.putMessage(Games.THANBAI.getId(), username, moneyExchange - (long)this.betValue);
                                        }
                                   }
                                   ...
                                   try {
                                        ...
                                        if (result == 3 || result == 2 || result == 4) {
                                             BigWinThanBaiMsg bigWinMsg = new BigWinThanBaiMsg();
     ...
     public ResultThanBaiMsg playFreeDaily(String username, long referenceId) {
                    ...
                    while(true) {
                         ...
                         if (totalPrizes <= (long)ConfigGame.getIntValue("max_prize_free_daily", 2000)) {
                              ...
                              if (!updated) {
                                   ...
                                   if (moneyExchange > 0L) {
                                        String des = "Quay ThanBai Free ";
                                        MoneyResponse moneyRes = this.userService.updateMoney(username, moneyExchange, this.moneyTypeStr, "ThanBaiFree", "Quay ThanBai Free ", "Cược: 0, Thắng: " + totalPrizes, 0L, (Long)null, TransType.NO_VIPPOINT);
                                        if (moneyRes != null && moneyRes.isSuccess()) {
                                             currentMoney = moneyRes.getCurrentMoney();
                                        }
                                   }
     ...
     public ResultThanBaiMsg playFree(String username, String linesStr, String itemsWild, int ratio, long referenceId) {
          long startTime = System.currentTimeMillis();
          String currentTimeStr = DateTimeUtils.getCurrentTime();
          short result = 0;
          String[] lineArr = linesStr.split(",");
          ...
          while(true) {
               ...
               long moneyExchange = totalPrizes - 0L;
               String des = "ThanBai - Free";
               MoneyResponse moneyRes = this.userService.updateMoney(username, moneyExchange, this.moneyTypeStr, this.gameName, "ThanBai - Free", this.buildDescription(0L, totalPrizes, result), 0L, (Long)null, TransType.VIPPOINT);
               if (moneyRes != null && moneyRes.isSuccess()) {
                    currentMoney = moneyRes.getCurrentMoney();
                    if (this.moneyType == 1 && moneyExchange >= (long)BroadcastMessageServiceImpl.MIN_MONEY) {
                         this.broadcastMsgService.putMessage(Games.THANBAI.getId(), username, moneyExchange);
                    }
               ...
               if (freeSpin.getNum() == 0) {
                    ThanBaiTotalFreeSpin totalFreeSpinMsg = new ThanBaiTotalFreeSpin();
     ...
     public short play(User user, String linesStr) {
          ...
          ResultThanBaiMsg msg = null;
          ThanBaiFreeDailyMsg freeDailyMsg = new ThanBaiFreeDailyMsg();
          if (numFree > 0) {
          ...
          if (this.isUserMinimize(user)) {
               MinimizeResultThanBaiMsg miniMsg = new MinimizeResultThanBaiMsg();
     ...
     private void savePot() {
          ...
          if (currentTime - this.lastTimeUpdatePotToRoom >= 3000L) {
               ...
               UpdatePotThanBaiMsg msg = new UpdatePotThanBaiMsg();
               msg.value = this.pot;
