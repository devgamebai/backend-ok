...
public final class BZUserManager extends BaseCoreService implements IUserManager {
     ...
     public void addUser(User user) {
          if (this.containsId(user.getUniqueId())) {
               ...
               // Remove stale sessions for the same username (duplicate login from another device)
               List oldUsers = (List)this.usersByName.get(user.getName());
               if (oldUsers != null && !oldUsers.isEmpty()) {
                    for (Object obj : new ArrayList(oldUsers)) {
