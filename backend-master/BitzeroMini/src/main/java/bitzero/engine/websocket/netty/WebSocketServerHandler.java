...
public class WebSocketServerHandler extends SimpleChannelUpstreamHandler implements IWebSocketChannel {
     ...
     public void write(ChannelBuffer message) {
          if (this.wsChannel == null || !this.wsChannel.isOpen()) {
              return;
          }
          try {
              // Manually encode WebSocket binary frame and write raw bytes
              // This bypasses the WebSocket13FrameEncoder which may not flush in Docker
              ChannelBuffer data = org.jboss.netty.buffer.ChannelBuffers.copiedBuffer(message);
              int dataLen = data.readableBytes();
              
              ChannelBuffer wsFrame;
              if (dataLen <= 125) {
                  wsFrame = org.jboss.netty.buffer.ChannelBuffers.buffer(2 + dataLen);
                  wsFrame.writeByte(0x82); // FIN + binary opcode
                  wsFrame.writeByte(dataLen);
              } else if (dataLen <= 65535) {
                  wsFrame = org.jboss.netty.buffer.ChannelBuffers.buffer(4 + dataLen);
                  wsFrame.writeByte(0x82);
                  wsFrame.writeByte(126);
                  wsFrame.writeShort(dataLen);
              } else {
                  wsFrame = org.jboss.netty.buffer.ChannelBuffers.buffer(10 + dataLen);
                  wsFrame.writeByte(0x82);
                  wsFrame.writeByte(127);
                  wsFrame.writeLong(dataLen);
              }
              wsFrame.writeBytes(data);
              
              // Send raw ChannelBuffer (NOT BinaryWebSocketFrame)
              // The pipeline will pass this through as raw bytes
              this.wsChannel.write(wsFrame);
          } catch (Exception e) {
              // silently ignore
          }
     }
